...
func TestPrintUsage(t *testing.T) {
	...
	if !strings.Contains(stdout, "aria-core vtest-version") {
	...
	if !strings.Contains(stdout, "Required for cloud serve in BOTH token auth and insecure no-auth mode") {
		t.Fatalf("usage missing updated ARIA_CORE_CLOUD_ALLOWED_PROJECTS contract: %q", stdout)
	}
	for _, token := range []string{
		"ARIA_CORE_DATABASE_URL",
		"ARIA_CORE_CLOUD_HOST",
...
func TestPrintPostInstall(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expects: []string{"Restart OpenCode", "opencode-subagent-statusline", "aria-core serve &"},
		},
		{
			name:       "opencode with subagent monitor not enabled",
			result:     &setup.Result{Agent: "opencode", TUIPluginEnabled: false},
			expects:    []string{"Restart OpenCode", "aria-core serve &"},
...
func TestCmdSyncCloudRegressionPreservesLegacyBehaviorWithUpgradeStatePresent(t *testing.T) {
	...
	syncExport = func(*coresync.Syncer, string, string) (*coresync.SyncResult, error) {
		return &coresync.SyncResult{ChunkID: "chunk-regression", SessionsExported: 1}, nil
	}
...
func TestCmdSaveAndSearch(t *testing.T) {
	...
	withArgs(t,
		"aria-core", "save", "my-title", "my-content",
		"--type", "bugfix",
		"--project", "alpha",
		"--scope", "personal",
		"--topic", "auth/token",
	...
	withArgs(t, "aria-core", "search", "my-content", "--type", "bugfix", "--project", "alpha", "--scope", "personal", "--limit", "1")
	...
	withArgs(t, "aria-core", "search", "definitely-not-found")
...
func TestCmdTimeline(t *testing.T) {
	...
	withArgs(t, "aria-core", "timeline", strconv.FormatInt(focusID, 10), "--before", "1", "--after", "1")
...
func TestCmdContextAndStats(t *testing.T) {
	...
	withArgs(t, "aria-core", "context", "project-x")
	...
	if !strings.Contains(statsOut, "AriaCore Memory Stats") || !strings.Contains(statsOut, "project-x") {
...
func TestCmdExportAndImport(t *testing.T) {
	...
	withArgs(t, "aria-core", "export", exportPath)
	...
	withArgs(t, "aria-core", "import", exportPath)
...
func TestMainVersionAndHelpAliases(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "version", arg: "version", contains: "aria-core 9.9.9-test", notStderr: true},
		{name: "version short", arg: "-v", contains: "aria-core 9.9.9-test", notStderr: true},
		{name: "version long", arg: "--version", contains: "aria-core 9.9.9-test", notStderr: true},
...
func TestMainPrintsUpdateFailuresAndUpdates(t *testing.T) {
	...
	t.Run("prints check failure", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "version")

		stdout, stderr := captureOutput(t, func() { main() })
		if !strings.Contains(stdout, "aria-core 1.10.7") {
...
func TestMainExitPaths(t *testing.T) {
	tests := []struct {
		...
		{name: "cloud missing subcommand", helperCase: "cloud-missing", expectedOutput: "usage: aria-core cloud", expectedExitOne: true},
		...
		{name: "cloud enroll missing project", helperCase: "cloud-enroll-missing", expectedOutput: "usage: aria-core cloud enroll <project>", expectedExitOne: true},
	}
...
func TestMainExitHelper(t *testing.T) {
	...
	case "no-args":
		os.Args = []string{"aria-core"}
	case "unknown":
		os.Args = []string{"aria-core", "definitely-unknown-command"}
	case "cloud-missing":
		os.Args = []string{"aria-core", "cloud"}
	case "cloud-unknown":
		os.Args = []string{"aria-core", "cloud", "nope"}
	case "cloud-enroll-missing":
		os.Args = []string{"aria-core", "cloud", "enroll"}
	default:
		os.Args = []string{"aria-core", "--help"}
	}
...
func TestCmdSearchLocalMode(t *testing.T) {
	...
	withArgs(t, "aria-core", "search", "local", "--project", "proj-local")
...
func TestCmdProjectsListEmpty(t *testing.T) {
	...
	withArgs(t, "aria-core", "projects", "list")
...
func TestCmdProjectsConsolidateNoSimilar(t *testing.T) {
	...
	withArgs(t, "aria-core", "projects", "consolidate")
...
func TestCmdProjectsConsolidateDryRun(t *testing.T) {
	...
	mustSeedObservation(t, cfg, "s-eng", "aria-core", "note", "eng note", "content", "project")
	mustSeedObservation(t, cfg, "s-engm", "aria-core-memory", "note", "engm note", "content", "project")

	old := detectProject
	detectProject = func(string) string { return "aria-core" }
	t.Cleanup(func() { detectProject = old })

	withArgs(t, "aria-core", "projects", "consolidate", "--dry-run")
	stdout, stderr := captureOutput(t, func() { cmdProjectsConsolidate(cfg) })
	if stderr != "" {
		t.Fatalf("expected no stderr, got: %q", stderr)
	}
...
func TestCmdProjectsConsolidateSingleProject(t *testing.T) {
	...
	// Verify aria-core-memory was merged into aria-core
	s, err := store.New(cfg)
	if err != nil {
		t.Fatalf("store.New: %v", err)
	}
	...
	if len(names) != 1 || names[0] != "aria-core" {
		t.Fatalf("expected only 'aria-core' after merge, got: %v", names)
	}
}
...
func TestCmdProjectsConsolidateAllDryRun(t *testing.T) {
	...
	withArgs(t, "aria-core", "projects", "consolidate", "--all", "--dry-run")
	stdout, stderr := captureOutput(t, func() { cmdProjectsConsolidate(cfg) })
	if stderr != "" {
		t.Fatalf("expected no stderr, got: %q", stderr)
	}
...
func TestCmdProjectsAllNoGroups(t *testing.T) {
	...
	withArgs(t, "aria-core", "projects", "consolidate", "--all")
	stdout, stderr := captureOutput(t, func() { cmdProjectsConsolidate(cfg) })
	if stderr != "" {
		t.Fatalf("expected no stderr, got: %q", stderr)
	}
...
func TestCmdMCPDetectsProjectFromFlag(t *testing.T) {
	...
	withArgs(t, "aria-core", "mcp", "--project=myproject")
...
func TestCmdMCPDetectsProjectFromEnv(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_PROJECT", "env-project")
...
func TestObsidianExportMissingVault(t *testing.T) {
	...
	withArgs(t, "aria-core", "obsidian-export", "--project", "eng")
...
func TestObsidianExportCallsInjectedExporter(t *testing.T) {
	...
	withArgs(t, "aria-core", "obsidian-export",
		"--vault", vaultDir,
		"--project", "eng",
		"--limit", "50",
		"--since", "2026-01-01",
...
func TestObsidianExportMinimalFlags(t *testing.T) {
	...
	withArgs(t, "aria-core", "obsidian-export", "--vault", vaultDir)
...
func TestObsidianExportWatchRequiresInterval(t *testing.T) {
	...
	withArgs(t, "aria-core", "obsidian-export", "--vault", vaultDir, "--watch")
