...
type fakeStatusProvider struct {
	status coresrv.SyncStatus
}

func (f *fakeStatusProvider) Status(_ string) coresrv.SyncStatus {
	return f.status
}
...
func TestSyncStatusAdapterHealthy(t *testing.T) {
	...
	fallback := &fakeStatusProvider{status: coresrv.SyncStatus{Phase: "degraded"}}
...
func TestSyncStatusAdapterNilFallback(t *testing.T) {
	...
	expected := coresrv.SyncStatus{Phase: "healthy", ReasonCode: "all-good"}
...
func TestSyncStatusAdapterOverlayUpgradeStage(t *testing.T) {
	...
	_ = now
	mgr := &fakeAutosyncManager{status: autosync.Status{Phase: autosync.PhaseHealthy}}
	fallback := &fakeStatusProvider{status: coresrv.SyncStatus{
		Phase:                "degraded",
