...
import (
	...
	"github.com/ITECHDEV-MX/aria-core/internal/cloud/autosync"
	"github.com/ITECHDEV-MX/aria-core/internal/cloud/constants"
	"github.com/ITECHDEV-MX/aria-core/internal/cloud/remote"
	"github.com/ITECHDEV-MX/aria-core/internal/mcp"
	coresrv "github.com/ITECHDEV-MX/aria-core/internal/server"
	"github.com/ITECHDEV-MX/aria-core/internal/setup"
	"github.com/ITECHDEV-MX/aria-core/internal/store"
	coresync "github.com/ITECHDEV-MX/aria-core/internal/sync"
	"github.com/ITECHDEV-MX/aria-core/internal/tui"
	versioncheck "github.com/ITECHDEV-MX/aria-core/internal/version"
...
func (s stubManifestReader) ReadManifest(context.Context, string) (*coresync.Manifest, error) {
	if s.err != nil {
		return nil, s.err
	}
	if s.manifest != nil {
...
func stubRuntimeHooks(t *testing.T) {
	...
	storeNew = store.New
	newHTTPServer = func(s *store.Store, _ int) *coresrv.Server { return coresrv.New(s, 0) }
	startHTTP = func(_ *coresrv.Server) error { return nil }
	...
	syncStatus = func(sy *coresync.Syncer) (localChunks int, remoteChunks int, pendingImport int, err error) {
		return sy.Status()
	}
	syncImport = func(sy *coresync.Syncer) (*coresync.ImportResult, error) { return sy.Import() }
	syncExport = func(sy *coresync.Syncer, createdBy, project string) (*coresync.SyncResult, error) {
...
func TestFatal(t *testing.T) {
	...
	if !strings.Contains(stderr, "aria-core: boom") {
...
func TestCmdServeParsesPortAndErrors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			stubExitWithPanic(t)
			if tc.envPort != "" {
				t.Setenv("ARIA_CORE_PORT", tc.envPort)
			} else {
				t.Setenv("ARIA_CORE_PORT", "")
			}

			args := []string{"aria-core", "serve"}
			if tc.argPort != "" {
			...
			seenPort := -1
			newHTTPServer = func(s *store.Store, port int) *coresrv.Server {
				seenPort = port
				return coresrv.New(s, 0)
			}
			startHTTP = func(_ *coresrv.Server) error {
				return tc.startErr
			}
...
func TestCmdServeAutosyncLifecycleGating(t *testing.T) {
	...
	t.Run("local-only serve does not start autosync", func(t *testing.T) {
		cfg := testConfig(t)
		t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "")
		...
		withArgs(t, "aria-core", "serve", "9011")
	...
	t.Run("cloud autosync env with token and server starts successfully", func(t *testing.T) {
		...
		cfg := testConfig(t)
		t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "1")
		t.Setenv("ARIA_CORE_CLOUD_TOKEN", "test-token")
		t.Setenv("ARIA_CORE_CLOUD_SERVER", "http://127.0.0.1:9999")
...
func TestAutosyncEnvAbsent(t *testing.T) {
	// REQ-210: ARIA_CORE_CLOUD_AUTOSYNC not set → autosync does not start.
	stubRuntimeHooks(t)
	stubExitWithPanic(t)
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "tok")
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "http://localhost:9999")

	withArgs(t, "aria-core", "serve", "9111")
...
func TestAutosyncEnvNotOne(t *testing.T) {
	// REQ-210: ARIA_CORE_CLOUD_AUTOSYNC=true (not "1") → autosync does not start.
	stubRuntimeHooks(t)
	stubExitWithPanic(t)
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "true")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "tok")
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "http://localhost:9999")
	...
	_, _, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
		t.Fatal("serve should not fatal when ARIA_CORE_CLOUD_AUTOSYNC=true (not '1')")
	}
}
...
func TestAutosyncGatingTokenMissing(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "1")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "")
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "http://localhost:9999")

	withArgs(t, "aria-core", "serve", "9112")
	_, _, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
...
func TestAutosyncGatingServerMissing(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_AUTOSYNC", "1")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "tok")
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "")

	withArgs(t, "aria-core", "serve", "9113")
	_, _, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
...
func TestAutosyncGatingBothPresent(t *testing.T) {
	...
	withArgs(t, "aria-core", "serve", "9114")
	_, _, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
...
func TestCmdServeStartsWithoutAutosync(t *testing.T) {
	...
	withArgs(t, "aria-core", "serve", "9115")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
...
func TestCloudCommandIsolationDoesNotMutateLocalState(t *testing.T) {
	...
	cfg := testConfig(t)
	cfg.DataDir = filepath.Join(tmpHome, ".aria-core")

	withArgs(t, "aria-core", "cloud", "status")
	_, _, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil {
		t.Fatalf("cloud status should not exit: %v", recovered)
	}

	if _, err := os.Stat(filepath.Join(cfg.DataDir, "aria-core.db")); err == nil {
...
func TestCmdCloudStatusDistinguishesAuthAndSyncReadiness(t *testing.T) {
	...
	t.Run("configured but missing token", func(t *testing.T) {
		t.Setenv("ARIA_CORE_CLOUD_TOKEN", "")
		t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "")
		withArgs(t, "aria-core", "cloud", "status")
	...
	t.Run("configured in insecure no-auth mode", func(t *testing.T) {
		t.Setenv("ARIA_CORE_CLOUD_TOKEN", "")
		t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "1")
		withArgs(t, "aria-core", "cloud", "status")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
			t.Fatalf("cloud status should succeed, panic=%v stderr=%q", recovered, stderr)
	...
	t.Run("configured with token", func(t *testing.T) {
		t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
		t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "")
		withArgs(t, "aria-core", "cloud", "status")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
...
func TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic(t *testing.T) {
	...
	t.Run("missing project fails loudly", func(t *testing.T) {
		withArgs(t, "aria-core", "cloud", "upgrade", "doctor")
	...
	t.Run("deterministic findings for unchanged state", func(t *testing.T) {
		withArgs(t, "aria-core", "cloud", "upgrade", "doctor", "--project", "proj-a")
	...
	t.Run("legacy payload gaps are surfaced by doctor and block bootstrap preflight", func(t *testing.T) {
		...
		db, err := sql.Open("sqlite", filepath.Join(cfg.DataDir, "aria-core.db"))
		if err != nil {
		...
		withArgs(t, "aria-core", "cloud", "upgrade", "doctor", "--project", "proj-legacy")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
			t.Fatalf("doctor should succeed, panic=%v stderr=%q", recovered, stderr)
		}
		...
		runUpgradeBootstrap = func(_ *store.Store, _ string, _ *cloudConfig) (*coresync.UpgradeBootstrapResult, error) {
			bootstrapCalled = true
			return &coresync.UpgradeBootstrapResult{Project: "proj-legacy", Stage: store.UpgradeStageBootstrapVerified}, nil
		}
		t.Cleanup(func() { runUpgradeBootstrap = oldBootstrap })

		withArgs(t, "aria-core", "cloud", "upgrade", "bootstrap", "--project", "proj-legacy")
...
func TestCmdCloudUpgradeBootstrapStatusAndRollbackSemantics(t *testing.T) {
	...
	t.Run("status shows stage and reason", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "cloud", "upgrade", "status", "--project", "proj-a")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
	...
	t.Run("rollback blocked after bootstrap verified", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "cloud", "upgrade", "rollback", "--project", "proj-a")
		_, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if _, ok := recovered.(exitCode); !ok {
	...
	t.Run("bootstrap resume flag accepted", func(t *testing.T) {
		oldBootstrap := runUpgradeBootstrap
		runUpgradeBootstrap = func(_ *store.Store, project string, _ *cloudConfig) (*coresync.UpgradeBootstrapResult, error) {
			return &coresync.UpgradeBootstrapResult{Project: project, Stage: store.UpgradeStageBootstrapVerified, Resumed: true, NoOp: false}, nil
		}
		t.Cleanup(func() { runUpgradeBootstrap = oldBootstrap })

		withArgs(t, "aria-core", "cloud", "upgrade", "bootstrap", "--project", "proj-a", "--resume")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
	...
	t.Run("bootstrap captures rollback snapshot before progression", func(t *testing.T) {
		captured := false
		oldBootstrap := runUpgradeBootstrap
		runUpgradeBootstrap = func(s *store.Store, project string, _ *cloudConfig) (*coresync.UpgradeBootstrapResult, error) {
			captured = true
			...
			return &coresync.UpgradeBootstrapResult{Project: project, Stage: store.UpgradeStageBootstrapVerified}, nil
		}
		t.Cleanup(func() { runUpgradeBootstrap = oldBootstrap })
...
func TestCmdCloudUpgradeRepairStatusAndRollbackBranches(t *testing.T) {
	...
	t.Run("repair requires project", func(t *testing.T) {
		cfg := testConfig(t)
		withArgs(t, "aria-core", "cloud", "upgrade", "repair")
		_, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if _, ok := recovered.(exitCode); !ok {
			t.Fatalf("expected fatal exit for missing --project, got %v", recovered)
		}
		if !strings.Contains(stderr, "usage: aria-core cloud upgrade repair") || !strings.Contains(stderr, "--project") {
	...
	t.Run("repair dry-run stays non-applied", func(t *testing.T) {
		cfg := testConfig(t)
		withArgs(t, "aria-core", "cloud", "upgrade", "repair", "--project", "proj-a")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
	...
	t.Run("repair apply flag is accepted", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "cloud", "upgrade", "repair", "--project", "proj-a", "--apply")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
...
func TestCmdCloudUpgradeHelpShowsGuidedWorkflow(t *testing.T) {
	...
	withArgs(t, "aria-core", "cloud", "upgrade", "--help")
	stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil || stderr != "" {
...
func TestCloudUpgradeDocsMatchHelpAndLocalFirstSemantics(t *testing.T) {
	...
	commandExamples := []string{
		"aria-core cloud upgrade doctor --project",
		"aria-core cloud upgrade repair --project",
		"aria-core cloud upgrade bootstrap --project",
		"aria-core cloud upgrade status --project",
	}
	...
	if !strings.Contains(strings.ToLower(agentSetup), "deferred") || !strings.Contains(agentSetup, "aria-core cloud") {
		t.Fatalf("AGENT-SETUP must describe deferred automation/manual cloud CLI flow")
	}
	if !strings.Contains(strings.ToLower(plugins), "deferred") || !strings.Contains(plugins, "aria-core cloud") {
...
func TestCloudDashboardDocsEnablementFlowIsExecutable(t *testing.T) {
	...
	for _, token := range []string{
		"aria-core cloud config --server",
		"aria-core cloud enroll smoke-project",
		"/dashboard/login",
		"/dashboard/contributors",
		"ARIA_CORE_CLOUD_TOKEN",
		"ARIA_CORE_JWT_SECRET",
		"ARIA_CORE_CLOUD_ADMIN",
	} {
	...
	withArgs(t, "aria-core", "cloud", "config", "--server", "http://127.0.0.1:18080")
	stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil || stderr != "" {
	...
	withArgs(t, "aria-core", "cloud", "enroll", "smoke-project")
	stdout, stderr, recovered = captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil || stderr != "" {
	...
	t.Setenv("ARIA_CORE_JWT_SECRET", strings.Repeat("x", 32))
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "smoke-project")
	t.Setenv("ARIA_CORE_CLOUD_ADMIN", "token-abc")
	...
	withArgs(t, "aria-core", "cloud", "serve")
	_, stderr, recovered = captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil || stderr != "" {
	...
	if seen.AdminToken != "token-abc" {
		t.Fatalf("expected ARIA_CORE_CLOUD_ADMIN to flow into runtime config, got %q", seen.AdminToken)
	}
}
...
func TestCmdCloudStatusHonorsEnvServerOverride(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "https://env-cloud.example.test")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "env-token")
...
func TestCmdCloudStatusRejectsInvalidEffectiveRuntimeServerURL(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "https://env-cloud.example.test?debug=1")
	withArgs(t, "aria-core", "cloud", "status")
	stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if _, ok := recovered.(exitCode); !ok {
...
func TestCmdCloudConfigRejectsInvalidServerURL(t *testing.T) {
	...
	for _, input := range tests {
		t.Run(input, func(t *testing.T) {
			withArgs(t, "aria-core", "cloud", "config", "--server", input)
			_, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
			if _, ok := recovered.(exitCode); !ok {
...
func TestCmdCloudConfigAcceptsValidServerURL(t *testing.T) {
	...
	cfg := testConfig(t)
	withArgs(t, "aria-core", "cloud", "config", "--server", "https://cloud.example.test")
	stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if recovered != nil || stderr != "" {
...
func TestCmdCloudServeStartsCloudRuntime(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_JWT_SECRET", strings.Repeat("x", 32))
	t.Setenv("ARIA_CORE_CLOUD_HOST", "0.0.0.0")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "proj-a")
...
func TestCmdCloudServeRequiresAuthTokenByDefault(t *testing.T) {
	...
	if !strings.Contains(stderr, "ARIA_CORE_CLOUD_TOKEN") {
...
func TestCmdCloudServeRequiresProjectAllowlistByDefault(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "")
	t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "")
	...
	if !strings.Contains(stderr, "ARIA_CORE_CLOUD_ALLOWED_PROJECTS") {
...
func TestCmdCloudServeInsecureModeDoesNotRequireJWTServiceStartup(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "proj-a")
	t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "1")
	t.Setenv("ARIA_CORE_JWT_SECRET", "short")
...
func TestCmdCloudServeInsecureModeRejectsDashboardAdminToken(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "proj-a")
	t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "1")
	t.Setenv("ARIA_CORE_CLOUD_ADMIN", "admin-token")
	...
	if !strings.Contains(stderr, "ARIA_CORE_CLOUD_ADMIN") || !strings.Contains(stderr, "ARIA_CORE_CLOUD_INSECURE_NO_AUTH") {
...
func TestCmdCloudServeAuthenticatedModeRequiresExplicitJWTSecret(t *testing.T) {
	...
	if !strings.Contains(stderr, "ARIA_CORE_JWT_SECRET") || !strings.Contains(stderr, "non-default") {
...
func TestCmdCloudServeAuthenticatedModeRejectsDefaultJWTSecret(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_JWT_SECRET", cloud.DefaultJWTSecret)
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
	t.Setenv("ARIA_CORE_CLOUD_ALLOWED_PROJECTS", "proj-a")
	t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "")
	...
	if !strings.Contains(stderr, "ARIA_CORE_JWT_SECRET") || !strings.Contains(stderr, "default") {
...
func TestUnconfiguredCloudKeepsLocalCommandDefaults(t *testing.T) {
	...
	if !strings.Contains(searchErr, "usage: aria-core search <query>") {
		t.Fatalf("unexpected search usage: %q", searchErr)
	}

	withArgs(t, "aria-core", "context")
	...
	withArgs(t, "aria-core", "sync", "--status")
...
func TestCmdServeWiresPersistedSyncStatusProvider(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
	t.Setenv("ARIA_CORE_PROJECT", "proj-a")
	s, err := store.New(cfg)
	if err != nil {
		t.Fatalf("store.New: %v", err)
	}
	...
	withArgs(t, "aria-core", "serve")

	newHTTPServer = func(s *store.Store, _ int) *coresrv.Server {
		return coresrv.New(s, 0)
	}
	startHTTP = func(srv *coresrv.Server) error {
...
func TestCmdCloudServeRejectsInsecureModeWhenTokenIsSet(t *testing.T) {
	...
	if !strings.Contains(stderr, "ARIA_CORE_CLOUD_INSECURE_NO_AUTH") || !strings.Contains(stderr, "ARIA_CORE_CLOUD_TOKEN") {
...
func TestStoreSyncStatusProviderRejectsInvalidRuntimeServerURL(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "https://cloud.example.test?debug=1")
...
func TestCmdSetupDirectAndInteractive(t *testing.T) {
	...
	withArgs(t, "aria-core", "setup", "codex")
	...
	withArgs(t, "aria-core", "setup", "broken")
...
func TestCmdExportDefaultAndCmdImportErrors(t *testing.T) {
	...
	if !strings.Contains(stdout, "Exported to aria-core-export.json") {
		t.Fatalf("unexpected default export output: %q", stdout)
	}
	if _, err := os.Stat(filepath.Join(workDir, "aria-core-export.json")); err != nil {
		t.Fatalf("expected default export file: %v", err)
	}

	badPath := filepath.Join(workDir, "missing", "out.json")
	withArgs(t, "aria-core", "export", badPath)
	...
	withArgs(t, "aria-core", "import")
	_, stderr, recovered = captureOutputAndRecover(t, func() { cmdImport(cfg) })
	if _, ok := recovered.(exitCode); !ok || !strings.Contains(stderr, "usage: aria-core import") {
		t.Fatalf("expected import usage exit, panic=%v stderr=%q", recovered, stderr)
	}

	withArgs(t, "aria-core", "import", filepath.Join(workDir, "nope.json"))
	_, stderr, recovered = captureOutputAndRecover(t, func() { cmdImport(cfg) })
	...
	withArgs(t, "aria-core", "import", invalidJSON)
	_, stderr, recovered = captureOutputAndRecover(t, func() { cmdImport(cfg) })
...
func TestMainDispatchServeMCPAndTUI(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_DATA_DIR", t.TempDir())
	withArgs(t, "aria-core", "serve", "8088")
...
func TestStoreInitFailurePaths(t *testing.T) {
	...
	argsByCmd := [][]string{
		...
		{"aria-core", "search", "q"},
		{"aria-core", "save", "t", "c"},
		{"aria-core", "timeline", "1"},
		{"aria-core", "context"},
		{"aria-core", "stats"},
		{"aria-core", "export"},
		{"aria-core", "import", importFile},
		{"aria-core", "sync"},
	}
...
func TestUsageAndValidationExits(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "search usage", args: []string{"aria-core", "search"}, run: cmdSearch, errSubstr: "usage: aria-core search"},
		{name: "search missing query", args: []string{"aria-core", "search", "--limit", "3"}, run: cmdSearch, errSubstr: "search query is required"},
		{name: "save usage", args: []string{"aria-core", "save", "title"}, run: cmdSave, errSubstr: "usage: aria-core save"},
		{name: "timeline usage", args: []string{"aria-core", "timeline"}, run: cmdTimeline, errSubstr: "usage: aria-core timeline"},
		{name: "timeline invalid id", args: []string{"aria-core", "timeline", "abc"}, run: cmdTimeline, errSubstr: "invalid observation id"},
	}
...
func TestMainDispatchRemainingCommands(t *testing.T) {
	...
	dataDir := t.TempDir()
	t.Setenv("ARIA_CORE_DATA_DIR", dataDir)
	...
	tests := []struct {
		...
		{name: "search", args: []string{"aria-core", "search", "focus"}},
		{name: "save", args: []string{"aria-core", "save", "t", "c"}},
		{name: "timeline", args: []string{"aria-core", "timeline", fmt.Sprintf("%d", focusID)}},
		{name: "context", args: []string{"aria-core", "context", "main-proj"}},
		{name: "stats", args: []string{"aria-core", "stats"}},
		{name: "export", args: []string{"aria-core", "export", filepath.Join(t.TempDir(), "exp.json")}},
		{name: "import", args: []string{"aria-core", "import", importFile}},
		{name: "sync", args: []string{"aria-core", "sync", "--all"}},
		{name: "setup", args: []string{"aria-core", "setup", "codex"}},
	}
...
func TestCmdSyncAdditionalBranches(t *testing.T) {
	...
	t.Run("status parse error", func(t *testing.T) {
		...
		if err := os.MkdirAll(filepath.Join(workDir, ".aria-core"), 0755); err != nil {
			t.Fatalf("mkdir .aria-core: %v", err)
		}
		if err := os.WriteFile(filepath.Join(workDir, ".aria-core", "manifest.json"), []byte("{bad json"), 0644); err != nil {
	...
	t.Run("import parse error", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "sync", "--import")
		_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
		if _, ok := recovered.(exitCode); !ok {
			t.Fatalf("expected fatal exit, got %v", recovered)
		}
...
func TestCmdSyncCloudPreflightMissingServerMarksConfigError(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("ARIA_CORE_CLOUD_SYNC", "1")

	withArgs(t, "aria-core", "sync", "--cloud", "--project", "proj-a")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if _, ok := recovered.(exitCode); !ok {
		t.Fatalf("expected fatal exit, got %v", recovered)
	}
...
func TestPreflightCloudSyncAllowsEmptyTokenWhenServerConfigured(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "http://127.0.0.1:9090")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "")
	t.Setenv("ARIA_CORE_CLOUD_INSECURE_NO_AUTH", "")
...
func TestCmdSyncCloudInvalidRuntimeServerURLMarkedConfigError(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "://bad-url")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "token-abc")
...
func TestCmdSyncCloudStatusTransportConfigErrorIsReadOnly(t *testing.T) {
	...
	withArgs(t, "aria-core", "sync", "--cloud", "--status", "--project", "proj-a")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if _, ok := recovered.(exitCode); !ok {
		t.Fatalf("expected fatal exit, got %v", recovered)
	}
...
func TestCmdSyncCloudStatusPrintsProjectScopedChunkCounts(t *testing.T) {
	...
	t.Setenv("ARIA_CORE_CLOUD_SERVER", "https://cloud.example.test")
	t.Setenv("ARIA_CORE_CLOUD_TOKEN", "")

	oldSyncStatus := syncStatus
	syncStatus = func(_ *coresync.Syncer) (int, int, int, error) {
		return 3, 7, 2, nil
	}
...
func TestCmdSyncCloudSuccessMarksTargetHealthy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "status",
			args: []string{"aria-core", "sync", "--cloud", "--status", "--project", "proj-a"},
			stub: func() {
				syncStatus = func(*coresync.Syncer) (int, int, int, error) {
					return 2, 2, 0, nil
				}
			},
			expectHealthy: false,
		...
		{
			name: "import",
			args: []string{"aria-core", "sync", "--cloud", "--import", "--project", "proj-a"},
			stub: func() {
				syncImport = func(*coresync.Syncer) (*coresync.ImportResult, error) {
					return &coresync.ImportResult{}, nil
				}
			},
			expectHealthy: true,
		},
		{
			name: "export",
			args: []string{"aria-core", "sync", "--cloud", "--project", "proj-a"},
			stub: func() {
				syncExport = func(*coresync.Syncer, string, string) (*coresync.SyncResult, error) {
					return &coresync.SyncResult{ChunkID: "chunk-test", SessionsExported: 1}, nil
				}
			},
			expectHealthy: true,
		},
...
func TestCmdSyncCloudImportKeepsPendingWhenLocalMutationsRemain(t *testing.T) {
	...
	withArgs(t, "aria-core", "sync", "--cloud", "--import", "--project", "proj-a")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if recovered != nil || stderr != "" {
...
func TestCmdSyncCloudExportKeepsPendingWhenLocalMutationsRemain(t *testing.T) {
	...
	syncExport = func(*coresync.Syncer, string, string) (*coresync.SyncResult, error) {
		return &coresync.SyncResult{ChunkID: "chunk-stub", SessionsExported: 1, ObservationsExported: 1}, nil
	}
	syncStatus = func(*coresync.Syncer) (int, int, int, error) {
		return 1, 1, 0, nil
	}
...
func TestCmdSyncCloudExportKeepsPendingWhenRemoteImportsRemain(t *testing.T) {
	...
	syncExport = func(*coresync.Syncer, string, string) (*coresync.SyncResult, error) {
		return &coresync.SyncResult{ChunkID: "chunk-stub", SessionsExported: 1}, nil
	}
	syncStatus = func(*coresync.Syncer) (int, int, int, error) {
		return 1, 2, 1, nil
	}
...
func TestCmdSyncCloudLifecycleStateIsProjectScoped(t *testing.T) {
	...
	syncExport = func(*coresync.Syncer, string, string) (*coresync.SyncResult, error) {
		return &coresync.SyncResult{ChunkID: "chunk-proj-b", SessionsExported: 1}, nil
	}
	syncStatus = func(*coresync.Syncer) (int, int, int, error) {
		return 1, 1, 0, nil
	}

	withArgs(t, "aria-core", "sync", "--cloud", "--project", "proj-b")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if recovered != nil || stderr != "" {
		t.Fatalf("expected successful cloud sync cycle, panic=%v stderr=%q", recovered, stderr)
	}
...
func TestCmdSyncCloudRequiresExplicitProjectAndRejectsAll(t *testing.T) {
	...
	withArgs(t, "aria-core", "sync", "--cloud", "--all")
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if _, ok := recovered.(exitCode); !ok {
		t.Fatalf("expected fatal exit, got %v", recovered)
	}
	...
	withArgs(t, "aria-core", "sync", "--cloud")
	_, stderr, recovered = captureOutputAndRecover(t, func() { cmdSync(cfg) })
	if _, ok := recovered.(exitCode); !ok {
		t.Fatalf("expected fatal exit, got %v", recovered)
	}
...
func TestCmdImportStoreImportFailure(t *testing.T) {
	...
	withArgs(t, "aria-core", "import", badImport)
	_, stderr, recovered := captureOutputAndRecover(t, func() { cmdImport(cfg) })
	if _, ok := recovered.(exitCode); !ok {
		t.Fatalf("expected fatal exit, got %v", recovered)
	}
...
func TestCmdSearchAndSaveDanglingFlags(t *testing.T) {
	...
	withArgs(t, "aria-core", "save", "dangling-title", "dangling-content", "--type")
	...
	withArgs(t, "aria-core", "search", "dangling-content", "--limit", "not-a-number", "--project")
...
func TestCmdSetupHyphenArgFallsBackToInteractive(t *testing.T) {
	...
	withArgs(t, "aria-core", "setup", "--not-an-agent")
...
func TestCmdTimelineNoBeforeAfterSections(t *testing.T) {
	...
	withArgs(t, "aria-core", "timeline", fmt.Sprintf("%d", focusID), "--before", "0", "--after", "0")
...
func TestCmdSyncImportEmptyAndMixedChunks(t *testing.T) {
	...
	t.Run("import with empty manifest", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(workDir, ".aria-core", "manifest.json"), []byte(`{"version":1,"chunks":[]}`), 0644); err != nil {
			t.Fatalf("write manifest: %v", err)
		}
...
func TestCommandErrorSeamsAndUncoveredBranches(t *testing.T) {
	...
	t.Run("search seam error", func(t *testing.T) {
		withArgs(t, "aria-core", "search", "needle")
	...
	t.Run("save seam error", func(t *testing.T) {
		withArgs(t, "aria-core", "save", "title", "content")
	...
	t.Run("timeline seam error", func(t *testing.T) {
		withArgs(t, "aria-core", "timeline", "1")
	...
	t.Run("sync uses explicit project flag", func(t *testing.T) {
		withCwd(t, t.TempDir())
		withArgs(t, "aria-core", "sync", "--project", "explicit-proj")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdSync(cfg) })
		if recovered != nil || stderr != "" {
...
func TestCmdMCP(t *testing.T) {
	...
	t.Run("--tools flag uses newMCPServerWithConfig with non-nil allowlist", func(t *testing.T) {
		...
		withArgs(t, "aria-core", "mcp", "--tools=agent")
		_, stderr, recovered := captureOutputAndRecover(t, func() { cmdMCP(cfg) })
		if recovered != nil || stderr != "" {
			t.Fatalf("expected clean run, got panic=%v stderr=%q", recovered, stderr)
		}
