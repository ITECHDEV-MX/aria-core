...
# AriaCore — Technical Reference

**Persistent memory for AI coding agents**

This is the complete technical reference for AriaCore. For getting started, see the [README](README.md). For per-agent setup, see [Agent Setup](docs/AGENT-SETUP.md).
...
| [AriaCore Cloud](docs/aria-core-cloud/README.md) | Cloud landing page, quickstart path, branding, and reference links |
...
| [Comparison](docs/COMPARISON.md) | Why AriaCore vs claude-mem |
...
AriaCore exposes two different runtimes. Keep routes split by runtime:

- **Local runtime (`aria-core serve`, JSON on `127.0.0.1:7437`)**
  - includes memory CRUD/search/context endpoints documented below
  - includes `GET /sync/status` (local node sync status)
- **Cloud runtime (`aria-core cloud serve`)**
...
Dashboard route tree (`aria-core cloud serve`):
...
AriaCore is local-first: local SQLite is authoritative; cloud features are optional replication/shared access and enrollment controls.

### Health

- `GET /health` — Returns `{"status": "ok", "service": "aria-core", "version": "<current>"}`
...
- `GET /sync/status` — Runtime sync-state status for the local node (`aria-core serve` only).
- In `aria-core serve`, sync status is wired to persisted SQLite sync state (project-scoped for detected/current project).
...
|---|---|---|
| `ARIA_CORE_DATA_DIR` | Override data directory | `~/.aria-core` |
| `ARIA_CORE_PORT` | Override HTTP server port | `7437` |
| `ARIA_CORE_PROJECT` | Override project name for MCP server | auto-detected via git |

### Cloud CLI (opt-in)

- `aria-core cloud status` — show current cloud config state plus auth/sync readiness without mutating local state
- `aria-core cloud enroll <project>` — enroll one project for cloud replication
- `aria-core cloud config --server <url>` — persist cloud server URL to `~/.aria-core/cloud.json`
- `aria-core cloud serve` — run cloud backend API + dashboard (`/dashboard`) using Postgres config from env
- `aria-core cloud upgrade doctor --project <project>` — deterministic read-only readiness diagnosis (`ready|blocked`, class/reason)
- `aria-core cloud upgrade repair --project <project> [--dry-run|--apply]` — deterministic local-safe repair planner/apply (no remote mutation)
- `aria-core cloud upgrade bootstrap --project <project> [--resume]` — resumable checkpointed enroll/push/verify flow
- `aria-core cloud upgrade status --project <project>` — show upgrade stage/class/reason
- `aria-core cloud upgrade rollback --project <project>` — restore pre-upgrade local snapshot before `bootstrap_verified`; blocked afterwards

Cloud auth token is provided at runtime via `ARIA_CORE_CLOUD_TOKEN` (not by a dedicated CLI subcommand).
Cloud server startup fails closed when the token is missing unless `ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1` is explicitly set for local insecure development.
`ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1` cannot be combined with `ARIA_CORE_CLOUD_TOKEN`.
Cloud server always requires `ARIA_CORE_CLOUD_ALLOWED_PROJECTS` (comma-separated), including insecure mode, so project scope remains server-enforced.
`ARIA_CORE_CLOUD_TOKEN` + `ARIA_CORE_CLOUD_ALLOWED_PROJECTS` are server-side requirements for authenticated mode and must be configured before `aria-core cloud serve` (or compose startup).
Authenticated mode also requires an explicit non-default `ARIA_CORE_JWT_SECRET`; implicit development defaults are rejected.
Dashboard requests support browser login in authenticated mode: use `/dashboard/login` to exchange the bearer token for an HttpOnly dashboard cookie scoped to `/dashboard`. Protected `/dashboard/*` HTML routes require that cookie and do **not** treat raw `Authorization: Bearer ...` headers as an authenticated browser session. Sync API routes (`/sync/pull`, `/sync/push`) remain header-auth only. In insecure mode (`ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1` + no `ARIA_CORE_CLOUD_TOKEN`), dashboard auth is bypassed and `/dashboard/login` redirects to `/dashboard/`.

`ARIA_CORE_CLOUD_ADMIN` is optional in authenticated mode; when set, `/dashboard/admin` is allowed only for sessions established with that exact token.
`ARIA_CORE_CLOUD_ADMIN` is rejected in insecure mode (`ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1`) to avoid an incoherent admin/browser auth path.

Cloud runtime bind host is controlled by `ARIA_CORE_CLOUD_HOST`:
- default: `127.0.0.1` (local-only, safer default)
- container/compose: set `ARIA_CORE_CLOUD_HOST=0.0.0.0` so published host ports can reach the cloud server

Cloud runtime envs for `aria-core cloud serve`:

| Variable | Required | Notes |
|---|---|---|
| `ARIA_CORE_DATABASE_URL` | yes | Postgres DSN for cloud chunk storage/dashboard read model |
| `ARIA_CORE_PORT` | no | Runtime port (default `8080`) |
| `ARIA_CORE_CLOUD_HOST` | no | Bind host (default `127.0.0.1`; use `0.0.0.0` for containers) |
| `ARIA_CORE_CLOUD_ALLOWED_PROJECTS` | yes | Comma-separated allowlist; always required (authenticated + insecure modes) |
| `ARIA_CORE_CLOUD_TOKEN` | yes (authenticated mode) | Enables bearer auth mode |
| `ARIA_CORE_JWT_SECRET` | yes (authenticated mode) | Must be explicitly set and non-default when token mode is enabled |
| `ARIA_CORE_CLOUD_INSECURE_NO_AUTH` | no | Set to `1` only for local insecure mode; cannot be combined with `ARIA_CORE_CLOUD_TOKEN` |
| `ARIA_CORE_CLOUD_ADMIN` | no | Optional admin dashboard token in authenticated mode; rejected in insecure mode |
...
ARIA_CORE_CLOUD_SYNC=1 aria-core sync --status --project my-project
```
...
# client-side ARIA_CORE_CLOUD_INSECURE_NO_AUTH flag is required for compose flow
unset ARIA_CORE_CLOUD_TOKEN
...
# aria-core sync --cloud --all  # blocked by design
```
...
`aria-core sync --cloud --status --project <name>` is read-only: it does **not** mutate `/sync/status` lifecycle fields.
...
AriaCore resolves the project for every MCP tool call from the **server's working directory** (cwd), not from the LLM's arguments. This eliminates project drift caused by agents supplying different names for the same repo.
...
```json
{
  ...
  "project_path": "/home/user/aria-core",
  "result": "...(tool output)..."
}
```
...
The Memory Protocol teaches agents **when** and **how** to use AriaCore's MCP tools. Without it, the agent has the tools but no behavioral guidance. Add this to your agent's prompt file (see [Agent Setup](docs/AGENT-SETUP.md) for per-agent locations).
...
When completing a task, include a `## Key Learnings:` section at the end of your response with numbered items. AriaCore will automatically extract and save these as observations.
...
AriaCore automatically prevents project name drift — the same project saved under different names (`"aria-core"` vs `"AriaCore"` vs `"aria-core-memory"`) by different clients or users.
...
1. `--project` flag
2. `ARIA_CORE_PROJECT` environment variable
...
When saving to a project that doesn't exist yet, AriaCore checks for similar existing project names (Levenshtein distance, substring, case-insensitive matching) and warns the agent if a likely variant already exists.

### Retroactive cleanup

Use `aria-core projects consolidate` to interactively merge variant project names, or `mem_merge_projects` for agent-driven consolidation.
...
- `aria-core export` — JSON dump of all sessions, observations, prompts
- `aria-core import <file>` — Load from JSON, sessions use INSERT OR IGNORE (skip duplicates), atomic transaction

### Git Sync (Chunked)

Share memories through git repositories using compressed chunks with a manifest index.

- `aria-core sync` — Exports new memories as a gzipped JSONL chunk to `.aria-core/chunks/`
- `aria-core sync --all` — Exports ALL memories from every project
- `aria-core sync --import` — Imports chunks listed in the manifest that haven't been imported yet
- `aria-core sync --status` — Shows how many chunks exist locally vs remotely (filesystem mode)
- `aria-core sync --cloud --status --project <name>` — Shows local, remote, and pending chunk counts for the specified cloud project
- `aria-core sync --project NAME` — Filters export to a specific project
...
│   └── ...
└── aria-core.db              <- local working DB (gitignored)
```

**Why chunks?**
- Each `aria-core sync` creates a NEW chunk — old chunks are never modified
...
Interactive Bubbletea-based terminal UI. Launch with `aria-core tui`.
...
2. Create directories: `mkdir -p ~/.aria-core ~/.config/systemd/user`
3. Create `~/.config/systemd/user/aria-core.service` (see below)
4. `systemctl --user daemon-reload`
5. `systemctl --user enable aria-core`
6. `systemctl --user start aria-core`
7. `journalctl --user -u aria-core -f`

```ini
[Unit]
Description=AriaCore Memory Server
After=network.target

[Service]
WorkingDirectory=%h
ExecStart=%h/.local/bin/aria-core serve
Restart=always
RestartSec=3
Environment=ARIA_CORE_DATA_DIR=%h/.aria-core
...
Autosync is a background push/pull replication service that keeps your local AriaCore store in sync with the AriaCore Cloud server without blocking local writes.

### Enabling Autosync

Autosync is **opt-in**. Set all three environment variables before starting `aria-core serve` or `aria-core mcp`:

| Variable | Required | Description |
|---|---|---|
| `ARIA_CORE_CLOUD_AUTOSYNC` | Yes (exact `"1"`) | Enables autosync. Any other value disables it. |
| `ARIA_CORE_CLOUD_TOKEN` | Yes | Bearer token for the cloud server. |
| `ARIA_CORE_CLOUD_SERVER` | Yes | Base URL of the cloud server (e.g. `https://cloud.aria-core.example.com`). |

Example:

```sh
ARIA_CORE_CLOUD_AUTOSYNC=1 \
ARIA_CORE_CLOUD_TOKEN=your-token \
ARIA_CORE_CLOUD_SERVER=https://cloud.aria-core.example.com \
aria-core serve
```

Missing `ARIA_CORE_CLOUD_TOKEN` or `ARIA_CORE_CLOUD_SERVER` logs an `ERROR` and disables autosync gracefully — the server still starts.
...
| `auth_required` | Bearer token rejected (401) | Rotate `ARIA_CORE_CLOUD_TOKEN` |
| `policy_forbidden` | Project access denied (403) | Check `ARIA_CORE_CLOUD_ALLOWED_PROJECTS` on the server |
...
**`transport_failed` with `server_unsupported` in logs**: The cloud server does not yet implement `POST /sync/mutations/push` or `GET /sync/mutations/pull`. Deploy a server version that includes these endpoints before enabling `ARIA_CORE_CLOUD_AUTOSYNC=1`. Check logs for the line containing `server_unsupported`.

**Autosync not starting**: Check that `ARIA_CORE_CLOUD_AUTOSYNC` is exactly `"1"` (not `"true"` or `"yes"`), and that both `ARIA_CORE_CLOUD_TOKEN` and `ARIA_CORE_CLOUD_SERVER` are non-empty. The process logs an `[autosync] ERROR` line explaining which variable is missing.
...
When project sync is paused and a push is rejected, AriaCore records an audit entry in `cloud_sync_audit_log`. This gives operators a persistent trail of every rejection event, visible in the admin dashboard under **Admin > Audit Log**.
...
- [Agent Setup](docs/AGENT-SETUP.md) — connect your agent to AriaCore
