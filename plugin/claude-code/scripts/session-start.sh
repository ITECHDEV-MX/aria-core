#!/bin/bash
# AriaCore — SessionStart hook for Claude Code
#
# 1. Ensures the aria-core server is running
# 2. Creates a session in aria-core
# 3. Auto-imports git-synced chunks if .aria-core/manifest.json exists
...
# Ensure aria-core server is running
if ! curl -sf "${ARIA_CORE_URL}/health" --max-time 1 > /dev/null 2>&1; then
  aria-core serve &>/dev/null &
  sleep 0.5
fi

# Migrate project name if it changed (one-time, idempotent)
if [ "$OLD_PROJECT" != "$PROJECT" ] && [ -n "$OLD_PROJECT" ] && [ -n "$PROJECT" ]; then
  curl -sf "${ARIA_CORE_URL}/projects/migrate" \
    -X POST \
...
if [ -n "$SESSION_ID" ] && [ -n "$PROJECT" ]; then
  curl -sf "${ARIA_CORE_URL}/sessions" \
    -X POST \
    -H "Content-Type: application/json" \
...
if [ -f "${CWD}/.aria-core/manifest.json" ]; then
  aria-core sync --import 2>/dev/null
fi

# Fetch memory context
ENCODED_PROJECT=$(printf '%s' "$PROJECT" | jq -sRr @uri)
CONTEXT=$(curl -sf "${ARIA_CORE_URL}/context?project=${ENCODED_PROJECT}" --max-time 3 2>/dev/null | jq -r '.context // empty')

# Inject Memory Protocol + context — stdout goes to Claude as additionalContext
cat <<'PROTOCOL'
## AriaCore Persistent Memory — ACTIVE PROTOCOL

You have aria-core memory tools. This protocol is MANDATORY and ALWAYS ACTIVE.
