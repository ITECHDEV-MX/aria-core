...
TypeScript Obsidian community plugin that provides a settings UI, a ribbon sync button, and HTTP-based incremental sync against the aria-core server, allowing users to trigger vault updates from inside Obsidian.
...
- GIVEN the user has AriaCore plugin installed in Obsidian
...
- **AriaCore URL** (string, default: `http://localhost:4444`): base URL of the aria-core server
- **Sync interval** (number, default: 0): auto-sync interval in minutes; 0 means manual only
- **Vault subfolder** (string, default: `aria-core`): subfolder inside the vault where notes are written
...
#### Scenario: User clears AriaCore URL

- GIVEN the user deletes the AriaCore URL value and saves
- WHEN a sync is triggered
- THEN an error notice "AriaCore URL is required" is shown
...
The plugin MUST add a ribbon button with a recognizable icon. Clicking it MUST trigger a manual sync against the configured AriaCore URL. While sync is in progress, the button MUST show a loading state. On completion, it MUST show a success or error notice.

#### Scenario: Successful manual sync

- GIVEN the plugin is configured with a valid AriaCore URL
- WHEN the user clicks the ribbon button
- THEN the button enters a loading state
- AND a sync request is sent to `{aria-core_url}/export`
...
- GIVEN the AriaCore server is not running
- WHEN the user clicks the ribbon button
- THEN a notice "Sync failed: could not reach aria-core server" is shown
- AND the button returns to idle state
...
1. Call `GET {aria-core_url}/export` (with optional query params: `project`, `since`)
...
- GIVEN the vault has no `aria-core/` subfolder
- WHEN a sync is triggered
- THEN the subfolder is created
- AND all exported notes are written to `{vault}/aria-core/`
...
- THEN `{vault}/aria-core/.../obs-7.md` is deleted from disk if it exists
