...
CLI command that reads the AriaCore SQLite store and writes a structured Obsidian-compatible markdown vault with YAML frontmatter, wikilinks, hub notes, and incremental sync state. Supports graph config bootstrap and a built-in watch-mode daemon for automatic periodic sync.
...
The `aria-core obsidian-export` subcommand MUST accept the following flags:
...
- GIVEN the user runs `aria-core obsidian-export --vault ~/vault --project eng --limit 100 --since 2026-01-01`
...
- GIVEN the user runs `aria-core obsidian-export --project eng`
- WHEN the command is parsed
...
- GIVEN the user runs `aria-core obsidian-export --unknown`
- WHEN the command is parsed
...
The exporter MUST create the following directory hierarchy inside `{vault}/aria-core/`:
...
All directories MUST be created if they do not exist. The exporter MUST NOT write outside the `{vault}/aria-core/` subdirectory.

#### Scenario: Fresh vault — directories don't exist

- GIVEN `--vault ~/vault` points to a directory without an `aria-core/` subfolder
- WHEN export runs
- THEN `~/vault/aria-core/` and all needed subdirectories are created
...
The exporter MUST persist the last-export timestamp in `{vault}/aria-core/.aria-core-sync-state.json`. On subsequent runs, it MUST only write observations created or updated after that timestamp.

#### Scenario: First export — no state file

- GIVEN `.aria-core-sync-state.json` does not exist
- WHEN export runs
- THEN all matching observations are exported
- AND `.aria-core-sync-state.json` is created with `last_export_at` set to the current time

#### Scenario: Incremental export — state file exists

- GIVEN `.aria-core-sync-state.json` has `last_export_at: "2026-03-01T00:00:00Z"`
...
- AND `.aria-core-sync-state.json` is updated to the new timestamp
...
The `aria-core obsidian-export` command MUST accept a `--graph-config` flag with accepted values: `preserve` (default), `force`, `skip`.
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v --graph-config=force`
- WHEN the flag is parsed
- THEN export proceeds with force mode

#### Scenario: Invalid flag value rejected

- GIVEN the user runs `aria-core obsidian-export --vault ./v --graph-config=invalid`
- WHEN the flag is parsed
...
When `--graph-config=force`, the system MUST always overwrite `{vault}/.obsidian/graph.json` with the embedded default and log "graph.json overwritten with aria-core default".
...
- AND "graph.json overwritten with aria-core default" is logged
...
| # | query | rgb |
|---|-------|-----|
| 1 | `path:aria-core/_sessions` | 14736466 |
| 2 | `path:aria-core/_topics` | 13893887 |
...
The `aria-core obsidian-export` command MUST accept a `--watch` flag.
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v`
- WHEN export completes
- THEN the process exits with code 0

#### Scenario: With --watch — loops until signal

- GIVEN the user runs `aria-core obsidian-export --vault ./v --watch`
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v --watch --interval 5m`
- WHEN the flag is parsed
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v --watch --interval 10m`
...
- AND the process exits with code 0
- AND `.aria-core-sync-state.json` is valid (not corrupted)
...
- GIVEN `--watch --project aria-core`
- WHEN the daemon runs multiple cycles
- THEN each cycle exports only observations from project "aria-core"
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v --interval 5m`
...
- GIVEN the user runs `aria-core obsidian-export --vault ./v --watch --interval 10s`
- WHEN the flags are parsed
