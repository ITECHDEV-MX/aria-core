...
### REQ-210: Explicit opt-in via ARIA_CORE_CLOUD_AUTOSYNC env var

Autosync MUST only start when `ARIA_CORE_CLOUD_AUTOSYNC=1` is set in the environment. Setting this env var without a valid token or server URL MUST NOT silently succeed (see REQ-211). Absence of the env var MUST leave autosync disabled with no log output beyond a single debug-level note. The env var MUST NOT auto-enable when only `ARIA_CORE_CLOUD_TOKEN` and `ARIA_CORE_CLOUD_SERVER` are set.

**Scenarios**:

- **Opt-in required — env absent**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC` is not set but token and server are set, WHEN `cmdServe` runs, THEN autosync does not start.
- **Opt-in present — autosync starts**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC=1`, token, and server URL are all set, WHEN `cmdServe` runs, THEN the Manager starts and a single info log confirms it.
- **Edge case — env value not "1"**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC=true`, WHEN `cmdServe` runs, THEN autosync does not start (only exact `"1"` is accepted).
- **Previous fatal inverted**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC=1` and valid config, WHEN `cmdServe` runs, THEN the process does NOT call `fatal()`; test `TestCmdServeAutosyncLifecycleGating` asserts the new behavior.

---

### REQ-211: Runtime gating — token and server URL required

When `ARIA_CORE_CLOUD_AUTOSYNC=1` is set but `ARIA_CORE_CLOUD_TOKEN` or `ARIA_CORE_CLOUD_SERVER` is missing, the runtime MUST log an error and skip starting autosync; it MUST NOT start the Manager with an empty token. The error MUST be logged at `ERROR` level and include which variable is missing. The `aria-core serve` process MUST still start successfully (autosync is optional; a missing token is not a fatal condition).

**Scenarios**:

- **Token missing — skip with error log**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC=1` and `ARIA_CORE_CLOUD_SERVER` set but `ARIA_CORE_CLOUD_TOKEN` empty, WHEN `cmdServe` runs, THEN autosync does not start and the log contains "ARIA_CORE_CLOUD_TOKEN".
- **Server URL missing — skip with error log**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC=1` and token set but `ARIA_CORE_CLOUD_SERVER` empty, WHEN `cmdServe` runs, THEN autosync does not start and the log contains "ARIA_CORE_CLOUD_SERVER".
...
- **Autosync disabled — store fallback**: GIVEN `ARIA_CORE_CLOUD_AUTOSYNC` is not set, WHEN `/sync/status` is queried, THEN the result comes from `storeSyncStatusProvider`.
