...
### Current State
- Integrated repo (`aria-core`) ships a **status-only** cloud dashboard:
...
- Original repo (`aria-core-cloud`) ships a **full server-rendered + htmx-enhanced dashboard**:
...
### Affected Areas
- `internal/cloud/dashboard/*` (aria-core + aria-core-cloud) — core parity surface and route/component behavior.
...
- `cmd/aria-core/cloud.go`, `internal/cloud/config.go` — runtime wiring for admin config and dashboard dependencies.
...
| File (source: aria-core-cloud) | Integrated state (aria-core) | Parity gap | Portability |
|---|---|---|---|
...
| `cmd/aria-core/cloud.go` | Uses `cloudserver.New(...)` with status provider; no `WithDashboard(cfg)` option | Extend runtime wiring for dashboard config and dependencies |
...
2. **dashboard auth boundary**
   - Old dashboard assumes identity-bearing JWT cookie (`aria-core_session`) from username/email/password login.
   - Integrated runtime uses bearer token (`ARIA_CORE_CLOUD_TOKEN`) + signed opaque dashboard cookie (`aria-core_dashboard_token`) that maps to one configured token.
...
4. **runtime/config wiring**
   - Ensure `ARIA_CORE_CLOUD_ADMIN` remains effective for admin gating in restored UI.
