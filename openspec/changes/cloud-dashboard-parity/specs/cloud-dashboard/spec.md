...
Restore standalone `aria-core-cloud` dashboard behavior in the integrated repo while preserving local-first and current security/runtime rules.
...
The system MUST preserve a styled `/dashboard/login` flow and adapt auth to integrated runtime boundaries (`ARIA_CORE_CLOUD_TOKEN`, signed dashboard cookie, `ARIA_CORE_JWT_SECRET` validation).
...
- GIVEN `ARIA_CORE_JWT_SECRET` is missing or invalid
