...
- [x] 3.2 dashboard middleware bridge with signed `aria-core_dashboard_token`
- [x] 3.3 admin gating bridged with `ARIA_CORE_CLOUD_ADMIN`
...
| verify-gap B | `cmd/aria-core/main_extra_test.go` | Integration (CLI/runtime operator flow) | ✅ `go test ./cmd/aria-core -run 'TestCloudUpgradeDocsMatchHelpAndLocalFirstSemantics|TestCmdCloudStatusHonorsEnvServerOverride|TestCmdCloudServeStartsCloudRuntime'` | ✅ added docs-backed runtime flow test spanning `config/status/enroll/serve` sequence | ✅ `go test ./cmd/aria-core -run TestCloudDashboardDocsEnablementFlowIsExecutable` | ✅ docs token verification + runtime env/auth/admin wiring assertions | ➖ none needed (test-only) |
...
- `go test ./cmd/aria-core -run TestCloudDashboardDocsEnablementFlowIsExecutable` ✅
