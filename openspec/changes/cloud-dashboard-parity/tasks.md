...
- [x] 1.3 GREEN: Restore UI sources from `aria-core-cloud` into `internal/cloud/dashboard/{layout.templ,components.templ,login.templ}` plus real `static/{styles.css,pico.min.css,htmx.min.js}`.
...
- [x] 3.2 GREEN: Add `internal/cloud/dashboard/middleware.go` adapting dashboard context/middleware to `aria-core_dashboard_token` + bearer-token validation (no legacy username/password reintroduction).
- [x] 3.3 GREEN: Reconcile admin gating with `ARIA_CORE_CLOUD_ADMIN` across `internal/cloud/dashboard` and `internal/cloud/config.go` expectations.
...
- [x] 5.1 Update `README.md`, `DOCS.md`, and `docs/ARCHITECTURE.md` with restored dashboard routes, env vars (`ARIA_CORE_CLOUD_TOKEN`, `ARIA_CORE_JWT_SECRET`, `ARIA_CORE_CLOUD_ADMIN`), and fallback behavior.
