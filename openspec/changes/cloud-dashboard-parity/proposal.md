...
The integrated repo only exposes a minimal sync-status dashboard, while `/Users/alanbuscaglia/work/aria-core-cloud` already contains the proven browser dashboard. Copy/reconcile that implementation first to preserve behavior and avoid redesign.

## Scope

### In Scope
- Restore dashboard routes, templates, partials, assets, and tests from `aria-core-cloud` into `internal/cloud/dashboard`.
- Reconcile missing `cloudstore` query/index APIs needed by overview, browser, projects, contributors, and admin pages.
- Adapt auth/runtime wiring to integrated constraints (`ARIA_CORE_CLOUD_TOKEN`, signed dashboard cookie, `ARIA_CORE_JWT_SECRET`, admin config).
...
- Phase 3: adapt auth/runtime seams: translate JWT-cookie assumptions to the integrated bearer-token session codec, preserve `/dashboard/login` POST fallback, and honor `ARIA_CORE_CLOUD_ADMIN` plus `ARIA_CORE_JWT_SECRET` rules.
...
| `cmd/aria-core/cloud.go`, `internal/cloud/config.go` | Modified | Support restored admin/auth runtime behavior. |
...
- Source implementation in `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/dashboard`
- Existing integrated auth/session codec in `internal/cloud/{auth,cloudserver}`

## Success Criteria

- [ ] Integrated repo serves the same dashboard surface area as `aria-core-cloud` without redefining product behavior.
