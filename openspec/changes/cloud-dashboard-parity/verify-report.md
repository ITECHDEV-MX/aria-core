...
| Integration (HTTP/handler/CLI runtime) | 11 (spec-mapped scenarios) | `internal/cloud/dashboard/dashboard_test.go`, `internal/cloud/cloudserver/cloudserver_test.go`, `cmd/aria-core/main_extra_test.go` | `go test` |
...
| Login and dashboard session flow adaptation | Invalid runtime secret configuration | `cmd/aria-core/main_extra_test.go > TestCmdCloudServeAuthenticatedModeRequiresExplicitJWTSecret` + `...RejectsDefaultJWTSecret` | ✅ COMPLIANT |
...
| Documentation parity | Operator follows docs to enable dashboard | `cmd/aria-core/main_extra_test.go > TestCloudDashboardDocsEnablementFlowIsExecutable` | ✅ COMPLIANT |
...
| Login and dashboard session flow adaptation | ✅ Implemented | Token validation + signed `aria-core_dashboard_token` session bridge enforced. |
...
| Auth/session model (`ARIA_CORE_CLOUD_TOKEN` + signed dashboard cookie) | ✅ Yes | Legacy username/password model not reintroduced. |
