...
| Auth/session model | Reintroduce username/password JWT flow; keep current bearer-token session bridge | Keep current `ARIA_CORE_CLOUD_TOKEN` + signed `aria-core_dashboard_token`; adapt dashboard middleware to inject a synthetic principal | Proposal explicitly rejects a new identity model. This preserves current security/runtime rules and keeps `/dashboard/login` as an HTTP form fallback. |
...
| `cmd/aria-core/cloud.go`, `internal/cloud/config.go`, `go.mod` | Modify | Wire `DashboardConfig`, admin env, and templ runtime dependency. |
...
- [ ] Whether unsupported account routes from `aria-core-cloud` (`change-password`) should be hidden entirely or left as explicit “not available in token mode” screens.
