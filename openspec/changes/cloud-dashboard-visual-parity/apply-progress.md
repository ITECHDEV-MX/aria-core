...
1. **gofmt on internal/store/store.go**: this file had a pre-existing formatting issue in the branch (`feat/integrate-aria-core-cloud`) before our changes. Not introduced by cloud-dashboard-visual-parity. Noted for sdd-verify phase awareness.
...
3. **TestLoginPageTokenFormAndCopy immediately passed** (no RED phase needed): The existing renderLoginPage function already had name="token", AriaCore Cloud, CLOUD ACTIVE, and name="next" — so this test was GREEN from the start. Recorded as exception in TDD evidence.
...
7. **Existing tests updated**: Several pre-existing tests in dashboard_test.go and cloudserver_test.go had stale assertions (testing old string-concat rendering behavior). Updated to match new templ-driven behavior:
   ...
   - TestHandlerDashboardLoginFlowSetsCookieForBrowserUse: updated "Dashboard Login" → "AriaCore Cloud" assertion
