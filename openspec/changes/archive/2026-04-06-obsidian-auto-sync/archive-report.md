...
Completed SDD cycle for the `obsidian-auto-sync` change, which extends `aria-core obsidian-export` with two new capabilities:
...
- `cmd/aria-core`: 25 tests (was 18, +7 new)
...
- `cmd/aria-core/main.go` — Modified (3 new flags, signal handling, watcher dispatch)
- `cmd/aria-core/main_test.go` — Modified (7 new test cases)
