...
### Out of Scope
- Integration with `aria-core serve` (daemon colocation is future work)
- `fsnotify`/inotify reactive file-watching mode
- Cross-vault or multi-vault sync
- Bidirectional sync (vault → aria-core)
...
3. In `cmd/aria-core/main.go`, parse `--graph-config`, `--watch`, `--interval`; wrap the single-run export in a `time.Ticker` loop when `--watch` is set; register `signal.NotifyContext(os.Interrupt, syscall.SIGTERM)` for graceful shutdown.
...
|------|--------|-------------|
| `cmd/aria-core/main.go` | Modified | Parse 3 new flags, daemon loop, signal handling |
...
Purely additive. Rollback = delete `internal/obsidian/graph.go`, `graph.json`, `graph_test.go`, and revert the flag parsing + daemon block in `cmd/aria-core/main.go`. No data migration, no store changes, existing single-run exports remain byte-identical.
...
- [ ] `aria-core obsidian-export --vault ./vault` writes `.obsidian/graph.json` on a fresh vault and leaves it alone on re-run
- [ ] `--graph-config=force` overwrites existing graph.json
- [ ] `--graph-config=skip` never touches `.obsidian/`
- [ ] `aria-core obsidian-export --vault ./vault --watch --interval 2m` runs first export immediately, then again after 2 minutes
