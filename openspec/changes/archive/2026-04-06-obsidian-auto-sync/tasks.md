...
- [x] 4.1 **[RED]** Modify `cmd/aria-core/main_test.go` — add cases: `--graph-config invalid` exits 1 with correct stderr; `--interval 5m` without `--watch` exits 1; `--watch --interval 30s` exits 1; `--watch --interval 2m --vault ...` invokes injected `newObsidianWatcher`. (REQ-GRAPH-01, REQ-WATCH-02, REQ-WATCH-07)
- [x] 4.2 **[GREEN]** Modify `cmd/aria-core/main.go` — parse `--graph-config` (default `preserve`), `--watch` (bool), `--interval` (default `10m`); validate (`ParseGraphConfigMode`, interval ≥ 1m, `--interval` requires `--watch`); add `var newObsidianWatcher = obsidian.NewWatcher` injectable; dispatch single-run vs `signal.NotifyContext(SIGINT, SIGTERM)` → `watcher.Run(ctx)` + log "shutting down watch mode".

## Phase 5: Documentation

- [x] 5.1 Modify `README.md` — add `--graph-config`, `--watch`, `--interval` rows to the flags table; add "Auto-sync" section with `aria-core obsidian-export --vault ~/Obsidian/aria-core --watch` example.
