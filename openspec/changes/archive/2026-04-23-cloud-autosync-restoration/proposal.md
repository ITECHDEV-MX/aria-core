...
Integrated `feat/integrate-aria-core-cloud` deliberately stubbed autosync — `ARIA_CORE_CLOUD_AUTOSYNC=1` fatals, `PushMutations`/`PullMutations` return "not available", and `cloudserver` has no `/sync/mutations/*` routes. Users must run explicit sync commands and teammate memories never flow back automatically. Restore the 451-line legacy manager from `aria-core-cloud`, adapt it to token auth + project-scoped enrollment + the dashboard status contract, and ship the missing cloudserver endpoints so local ↔ cloud sync runs silently in the background.

## Scope

### In Scope
- Port `internal/cloud/autosync/manager.go` from aria-core-cloud (phases, lease, debounce, backoff, push+pull loop).
...
- Replace the `fatal()` in `cmd/aria-core/main.go` with a `tryStartAutosync(s, cfg)` wiring serve + MCP.
...
| 7 | Auto-enable vs opt-in | Keep explicit opt-in: `ARIA_CORE_CLOUD_AUTOSYNC=1` required even when token+server are set. | Safer rollout; prevents surprise sync on existing setups after upgrade. Revisit for opt-out in a follow-up once stable. |
| 8 | Deployment ordering | Server endpoints MUST deploy to `aria-core.condetuti.com` BEFORE clients enable autosync. Document this in `DOCS.md` rollout section and add a startup log warning when `ARIA_CORE_CLOUD_AUTOSYNC=1` but first push returns 404 (→ `PhaseBackoff` with reason `server_unsupported`). | Client auto-disables loudly instead of failing silently, per aria-core-business-rules. |
...
| `cmd/aria-core/main.go` | Modified | Delete fatal; add `tryStartAutosync` + `autosyncStatusAdapter`. |
| `cmd/aria-core/main_extra_test.go` | Modified | Invert `TestCmdServeAutosyncLifecycleGating`; add healthy-start + shutdown tests. |
...
| Env semantic flip (`ARIA_CORE_CLOUD_AUTOSYNC=1` was fatal, now opt-in) | Med | Inverted tests explicitly assert new semantics; CHANGELOG/DOCS call out the shift. |
...
Revert the batch commits in reverse order: (5) docs, (4) cmdServe wiring → restore fatal, (3) status adapter, (2) manager port → restore stub, (1) server endpoints + transport. Each batch is independently revertable because earlier batches add server surface that is backwards compatible (new routes, no schema changes). If only client misbehaves, flip `ARIA_CORE_CLOUD_AUTOSYNC` off — server endpoints stay deployed harmlessly.

## Dependencies

- Legacy source: `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/autosync/manager.go` + `transport.go`.
...
- Server-side deploy to `aria-core.condetuti.com` before widespread client enablement.

## Success Criteria

- [ ] `ARIA_CORE_CLOUD_AUTOSYNC=1` + token + server → background loop starts in serve and MCP; no fatal.
