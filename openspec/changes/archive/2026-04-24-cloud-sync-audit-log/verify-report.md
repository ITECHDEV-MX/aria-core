...
- `go test -count=1 -race ./internal/cloud/... ./cmd/aria-core/...` — zero races

**go vet**: PASS — `go vet ./...` exits 0, no issues.

**gofmt**: PASS for all change-specific files. One pre-existing formatting issue found in `cmd/aria-core/autosync_status_test.go` (alignment of method stubs in `fakeAutosyncManager`) — this file was last touched in commit `5d2f9d7` which predates this change. Not introduced by this change.
...
| E2E test location | YES (deviation documented) | cloudserver package with fakeAuditableStoreForE2E instead of cmd/aria-core |
