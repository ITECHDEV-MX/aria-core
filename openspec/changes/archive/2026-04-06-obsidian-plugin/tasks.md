# Tasks: Obsidian Plugin for AriaCore Brain Visualization
...
## Phase 3: CLI Command — `cmd/aria-core/main.go`

- [x] 3.1 **[RED]** Write CLI tests in `cmd/aria-core/main_test.go` — missing `--vault` exits with code 1, unknown flag exits 1, valid flags call injected exporter (REQ-EXPORT-01)
...
- [x] 4.2 Implement `plugin/obsidian/src/settings.ts` — `AriaCoreSettings` interface, defaults (`url`, `syncInterval`, `subfolder`), `AriaCoreSettingTab` class with `loadData`/`saveData`; interval restart on change (REQ-PLUGIN-02)
- [x] 4.3 Implement `plugin/obsidian/src/main.ts` — `AriaCoreBrainPlugin` class, `onload()` registers ribbon button + settings tab + status bar item + optional polling, `onunload()` clears interval (REQ-PLUGIN-01, REQ-PLUGIN-03, REQ-PLUGIN-05)
