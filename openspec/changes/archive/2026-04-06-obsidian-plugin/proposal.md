...
Users have no way to visualize how their AI agent's memories connect. AriaCore stores rich relational data (topic_key hierarchies, session grouping, project isolation, type clustering) but it's invisible. Obsidian's native graph view can surface these connections as an interactive knowledge graph — if we write real `.md` files with wikilinks.

## Scope

### In Scope
- **`aria-core obsidian-export` CLI command** — new subcommand on existing binary
- **Markdown vault generation** — observations as notes with YAML frontmatter + wikilinks
- **Folder structure**: `{vault}/aria-core/{project}/{type}/{observation-slug}.md`
...
- Bidirectional sync (Obsidian → AriaCore writes)
...
- `obsidian-plugin`: TypeScript Obsidian community plugin providing settings UI, ribbon sync, and HTTP-based incremental sync against the aria-core server
...
- New `cmdObsidianExport(cfg)` in `cmd/aria-core/main.go` switch statement
...
- Incremental: store last-export timestamp in `{vault}/aria-core/.aria-core-sync-state.json`
...
- Settings tab: aria-core server URL, auto-sync interval, vault subfolder
...
|------|--------|-------------|
| `cmd/aria-core/main.go` | Modified | Add `obsidian-export` case + `cmdObsidianExport()` function |
...
- Generated vault files are in a subfolder — users delete `{vault}/aria-core/`
...
- [ ] `aria-core obsidian-export --vault ~/my-vault` generates a browsable Obsidian vault
