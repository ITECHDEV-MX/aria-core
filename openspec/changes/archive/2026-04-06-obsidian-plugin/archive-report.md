...
## AriaCore Observation IDs (Audit Trail)
...
| `cmd/aria-core/main.go` | `obsidian-export` switch case + cmdObsidianExport() |
...
| `plugin/obsidian/src/settings.ts` | AriaCoreSettings interface, AriaCoreSettingTab class |
| `plugin/obsidian/src/sync.ts` | HTTP sync client — GET /export, diff, vault.create/modify/delete |
| `plugin/obsidian/src/main.ts` | AriaCoreBrainPlugin class — ribbon, status bar, polling |
...
- **AriaCore wins** — one-way mirror design eliminates all conflict resolution complexity
