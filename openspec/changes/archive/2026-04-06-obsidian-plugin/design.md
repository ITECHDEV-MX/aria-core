# Technical Design: Obsidian Plugin for AriaCore Brain Visualization

## Executive Summary

Add Obsidian visualization to AriaCore via two deliverables: (1) a Go CLI exporter (`aria-core obsidian-export`) that reads SQLite and writes a markdown vault with frontmatter + wikilinks, and (2) an optional TypeScript Obsidian community plugin that provides a settings UI, ribbon button, and HTTP API sync mode.
...
### CLI Wiring (cmd/aria-core/main.go)
...
import "github.com/ITECHDEV-MX/aria-core/internal/store"
...
store.Export()
  → ExportData{Sessions[], Observations[], Prompts[]}
    ...
    → For each observation:
        ...
        2. Determine folder: {vault}/aria-core/{project}/{type}/
...
```
{vault}/
  aria-core/                              ← root namespace (MUST NOT write outside)
    .aria-core-sync-state.json            ← hidden state file
...
**Project**: aria-core | **Started**: 2026-04-06 14:00 | **Ended**: 15:30
...
```
1. Read SyncState from {vault}/aria-core/.aria-core-sync-state.json
...
**Conflict resolution**: AriaCore always wins. If user manually edits a vault file, next export overwrites it. This is a one-way mirror by design.
...
interface AriaCoreSettings {
    aria-coreUrl: string;      // default: "http://127.0.0.1:7437"
    autoSyncMinutes: number; // default: 0 (disabled)
    projectFilter: string;   // default: "" (all)
}

export default class AriaCoreBrainPlugin extends Plugin {
    settings: AriaCoreSettings;
    syncInterval: number | null;
    
    async onload() {
        await this.loadSettings();
        this.addSettingTab(new AriaCoreSettingTab(this.app, this));
        this.addRibbonIcon('brain', 'Sync AriaCore Brain', () => this.syncNow());
        this.addCommand({ id: 'sync-aria-core', name: 'Sync AriaCore Brain', callback: () => this.syncNow() });
        this.addStatusBarItem().setText('AriaCore: ready');
        if (this.settings.autoSyncMinutes > 0) this.startAutoSync();
    }
    
    async syncNow() {
        // 1. GET {aria-coreUrl}/export
        // 2. Read .aria-core-sync-state.json from vault
...
- `fetch(settings.aria-coreUrl + '/export')` → JSON
- Same diff logic against state file
- Uses Obsidian's `vault.create()` / `vault.modify()` / `vault.delete()` API
- Never touches files outside `aria-core/` folder

### Settings Tab

- AriaCore URL (text input, validated on save with `/health` check)
...
| Vault namespace | `{vault}/aria-core/` | `{vault}/` root | Security boundary; never clobber user notes |
...
| State file location | Inside vault | `~/.aria-core/` | Portable with vault; Git-trackable |
| Conflict resolution | AriaCore wins | Merge | One-way mirror = simple, predictable |
