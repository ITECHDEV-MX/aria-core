# Proposal: Integrate AriaCore Cloud

## Intent

Bring the proven cloud subsystem into `aria-core` without changing the product story: local SQLite remains primary, cloud is optional replication/shared access. This change creates the integration seam now and defers script validation until the cloud path is stable.

## Scope

### In Scope
- Import `internal/cloud/{auth,cloudstore,cloudserver,remote,autosync,dashboard}` into this repo with normalized module paths.
- Wire explicit `aria-core cloud ...` entrypoints and cloud config in `cmd/aria-core/main.go` while preserving existing local defaults.
...
|------|--------|-------------|
| `cmd/aria-core/main.go` | Modified | Add cloud command/config wiring without changing local defaults. |
...
Revert cloud command wiring in `cmd/aria-core/main.go`, disable autosync startup, and remove imported `internal/cloud/*` packages if regressions appear. Because local paths remain default and untouched by config, rollback restores current behavior cleanly.

## Dependencies

- Cloud subsystem source from `aria-core-cloud`
...
- [ ] `aria-core cloud ...` commands initialize and report deterministic failures.
