...
- `internal/cloud/**` (to be imported from `aria-core-cloud`) — cloudstore, cloudserver, auth, remote transport, autosync, dashboard packages.
...
### Approaches
1. **Big-bang parity merge from `aria-core-cloud`** — port all cloud-related diffs at once (commands, cloud packages, setup/docs, plugin scripts, dashboard).
   - Pros: Fastest route to feature parity with `aria-core-cloud`; fewer intermediate states.
   - Cons: Very high blast radius in `cmd`, setup/plugin/docs; hard to isolate regressions; violates “preserve current aria-core behavior” safety intent.
   - Effort: **High**
...
3. **Sidecar/bridge integration** — keep cloud server in separate repo/runtime and only add minimal client commands in this repo.
   - Pros: Minimal immediate code churn in `aria-core`; low local regression risk.
...
2. **Phase B — Explicit cloud commands**: add `aria-core cloud ...` entrypoints and cloud config handling in `cmd/aria-core/main.go`; keep `serve`, `mcp`, `search`, `context`, `sync` default-local unless explicit remote/cloud flags are used.
...
### Risks
- **Command-surface regression** in `cmd/aria-core/main.go` can break existing local flows if defaults change.
