# Tasks: Integrate AriaCore Cloud

## Phase 1: Foundation Import & Conflict Reconciliation

- [x] 1.1 **RED** Add non-regression tests in `cmd/aria-core/main_test.go` + `cmd/aria-core/main_extra_test.go` proving unconfigured cloud keeps `serve/mcp/search/context/sync` behavior and exit codes unchanged.
...
- [x] 1.4 Reconcile conflicting files: update `go.mod`/`go.sum`, resolve package-name collisions (`server` vs `cloudserver`), and record deferred script-touch points in `openspec/changes/integrate-aria-core-cloud/design.md` as explicit non-goals.
...
- [x] 2.1 **RED** Add CLI tests in `cmd/aria-core/main_test.go` for help discovery (`aria-core cloud` visible), unknown subcommand failure text, and required-arg errors.
...
- [x] 2.3 **GREEN** Implement `aria-core cloud ...` command tree and cloud config/env parsing in `cmd/aria-core/main.go` with actionable stderr on invalid invocations.
...
- [x] 4.1 **RED** Add startup wiring tests in `cmd/aria-core/main_test.go` proving autosync manager starts only for cloud-enabled long-lived processes and stays off for local-only runs.
- [x] 4.2 **GREEN** Wire cloud autosync lifecycle in `cmd/aria-core/main.go` using `internal/cloud/autosync` behind explicit enablement + enrollment checks.
...
- [x] 5.1 Update `README.md`, `DOCS.md`, and cloud-relevant `docs/*` pages with executable `aria-core cloud ...` examples, config keys, enrollment opt-in, and local-first source-of-truth statement.
...
- [x] 5.3 Run focused verification: `go test ./cmd/aria-core ./internal/sync ./internal/store ./internal/server ./internal/cloud/...` plus parity checks for unconfigured, unenrolled, auth-failed, and transport-failed paths.
...
- [x] 6.1 **RED** Add CLI/runtime tests proving `aria-core cloud serve` is discoverable and starts a cloud runtime path without mutating local-first defaults.
- [x] 6.2 **GREEN** Implement `aria-core cloud serve` runtime wiring through `cloudserver + cloudstore + auth` with actionable startup failures.
