...
**Change**: integrate-aria-core-cloud  
**Mode**: Strict TDD  
...
All tasks in `openspec/changes/integrate-aria-core-cloud/tasks.md` are marked complete.
...
curl -sS -o /tmp/aria-core-health.json -w "%{http_code}" "http://127.0.0.1:18080/health"
curl -sS -o /tmp/aria-core-dashboard.html -w "%{http_code}" "http://127.0.0.1:18080/dashboard"
curl -sS -o /tmp/aria-core-push.json -w "%{http_code}" -H "Authorization: Bearer smoke-token" -H "Content-Type: application/json" -X POST "http://127.0.0.1:18080/sync/push" --data '{"chunk_id":"compose-bind-verify-1","created_by":"verify","data":{"sessions":[{"id":"s-1"}]}}'
curl -sS -o /tmp/aria-core-pull-manifest.json -w "%{http_code}" -H "Authorization: Bearer smoke-token" "http://127.0.0.1:18080/sync/pull"
curl -sS -o /tmp/aria-core-pull-chunk.json -w "%{http_code}" -H "Authorization: Bearer smoke-token" "http://127.0.0.1:18080/sync/pull/compose-bind-verify-1"
curl -sS -o /tmp/aria-core-pull-unauth.txt -w "%{http_code}" "http://127.0.0.1:18080/sync/pull"

docker compose -f "docker-compose.cloud.yml" down --remove-orphans
```

Observed host results:
- `/health` → **200** (`{"service":"aria-core-cloud","status":"ok"}`)
...
Conclusion: the previous compose caveat is resolved; host → container reachability now works with `ARIA_CORE_CLOUD_HOST=0.0.0.0` in compose.
...
| RED confirmed (tests exist) | ✅ | Referenced files exist (`internal/cloud/config_test.go`, `internal/cloud/cloudserver/cloudserver_test.go`, `cmd/aria-core/main_extra_test.go`) |
...
|-------|-------|-------|-------|
| Unit | present | `internal/cloud/config_test.go`, `internal/cloud/cloudserver/cloudserver_test.go`, `cmd/aria-core/main_extra_test.go`, plus existing cloud suites | `go test` |
...
| `cmd/aria-core/cloud.go` (`cmdCloudServe` path) | 62.5% | ⚠️ Acceptable |
...
Reviewed changed test files for strict-TDD anti-patterns (`internal/cloud/config_test.go`, `internal/cloud/cloudserver/cloudserver_test.go`, relevant block in `cmd/aria-core/main_extra_test.go`):
...
| REQ-CLOUD-02 | ✅ Implemented | Explicit `aria-core cloud` surface present |
