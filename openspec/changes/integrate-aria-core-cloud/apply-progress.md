# Apply Progress — integrate-aria-core-cloud
...
- [x] 2.3 implemented `aria-core cloud ...` command tree with actionable stderr
- [x] 2.4 moved cloud parsing/validation into dedicated `cmd/aria-core/cloud.go`
...
- [x] 4.2 cloud autosync lifecycle wiring in `cmd/aria-core/main.go` behind explicit enablement and enrollment checks
...
|------|-----------|-------|------------|-----|-------|-------------|----------|
| 1.1 | `cmd/aria-core/main_test.go`, `cmd/aria-core/main_extra_test.go` | Unit | ✅ `go test ./cmd/aria-core -run "TestPrintUsage|TestMainUnknownCommand|TestMainUsageAndHelp|TestMainSyncSubcommands"` | ✅ Added failing expectations for `cloud` usage + cloud-related exit cases + unconfigured local-path regression | ✅ `go test ./cmd/aria-core -run "TestPrintUsage|TestMainExitPaths|TestCloudCommandIsolationDoesNotMutateLocalState|TestUnconfiguredCloudKeepsLocalCommandDefaults"` | ✅ multiple command paths (`serve/mcp/search/context/sync`) and multiple error branches | ✅ extracted cloud command flow into separate file to keep main readable |
...
| 1.4 | `openspec/changes/integrate-aria-core-cloud/design.md` | Doc/Design | N/A | ✅ explicit non-goal section absent before edit | ✅ verified file update | ➖ structural/doc task | ✅ deferred script touch points made explicit |
...
| 2.1 | `cmd/aria-core/main_test.go` | Unit | ✅ baseline cmd package test run | ✅ tests for cloud discoverability/invalid/missing args added first | ✅ cmd tests passing | ✅ multiple invocation shapes (`cloud`, `cloud nope`, `cloud enroll`) | ✅ helper-based exit coverage kept in existing style |
| 2.2 | `cmd/aria-core/main_extra_test.go` | Unit | ✅ baseline cmd package test run | ✅ isolation test fails before `cmdCloud` exists | ✅ passes after implementation | ✅ checks both DB and cloud config mutation absence | ➖ none needed |
| 2.3 | `cmd/aria-core/main.go`, `cmd/aria-core/cloud.go` + cmd tests | Unit | ✅ cmd suite baseline | ✅ invalid command path tests added first | ✅ `go test ./cmd/aria-core` | ✅ status/enroll/config branches + validation errors | ✅ command tree moved out of `main.go` |
| 2.4 | `cmd/aria-core/cloud.go` | Unit | N/A | ✅ refs to command helpers missing before split | ✅ compile + tests pass after split | ➖ structural split task | ✅ dedicated cloud command module |
...
| 3.5 | `cmd/aria-core/main.go` | Unit | ✅ cmd baseline (`TestCmdServeParsesPortAndErrors`, `TestUnconfiguredCloudKeepsLocalCommandDefaults`) | ✅ new tests failed on missing shared preflight/autosync seams | ✅ `go test ./cmd/aria-core -run "TestCmdServeAutosyncLifecycleGating|TestCmdSyncCloudPreflightAuthRequired"` | ✅ same helper now used by sync cloud mode and autosync startup gate | ✅ single `preflightCloudSync` owner for auth + enrollment semantics |
| 4.1 | `cmd/aria-core/main_extra_test.go` | Unit | ✅ cmd baseline above | ✅ startup lifecycle tests introduced before runtime wiring changes | ✅ targeted cmd tests pass for local-off/cloud-on lifecycle gating | ✅ both disabled and enabled+enrolled branches covered | ✅ seam-based manager injection avoids brittle process-level integration |
| 4.2 | `cmd/aria-core/main.go` | Unit | ✅ `TestCmdServeParsesPortAndErrors` baseline | ✅ no autosync manager seam existed for gating tests | ✅ serve path now conditionally starts manager and wires `NotifyDirty` | ✅ cloud enablement/env + enrollment gate + config/auth gate all validated via tests | ✅ store-backed sync status provider added without moving server ownership boundaries |
...
- **Targeted commands executed (this pass)**:
  ...
  - `go test ./cmd/aria-core -run "TestCmdServeParsesPortAndErrors|TestUnconfiguredCloudKeepsLocalCommandDefaults|TestCmdSyncAdditionalBranches"`
  - `go test ./cmd/aria-core -run "TestCmdServeAutosyncLifecycleGating|TestCmdSyncCloudPreflightAuthRequired"`
  - `go test ./cmd/aria-core -run "TestUnconfiguredCloudKeepsLocalCommandDefaults|TestCmdSyncAdditionalBranches|TestCmdServeParsesPortAndErrors"`
  - `go test ./internal/server -run "TestSyncStatusIncludesReasonParityFields|TestSyncStatusDegraded|TestSyncStatusNotConfigured"`
  - `go test ./cmd/aria-core ./internal/sync ./internal/store ./internal/server ./internal/cloud/...`
...
- [x] 6.1 RED CLI/runtime tests for `aria-core cloud serve` discovery and startup wiring.
- [x] 6.2 GREEN implementation of runnable `aria-core cloud serve` path with cloud runtime constructor seam.
...
| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|------|-----------|-------|------------|-----|-------|-------------|----------|
| 6.1 | `cmd/aria-core/main_extra_test.go`, `cmd/aria-core/main_test.go` | Unit | ✅ `go test ./cmd/aria-core -run "TestCloudCommandIsolationDoesNotMutateLocalState|TestPrintUsage"` | ✅ Added failing test requiring `cloud serve` runtime seam + usage line | ✅ `go test ./cmd/aria-core -run "TestCmdCloudServeStartsCloudRuntime|TestPrintUsage"` | ✅ startup path + discoverability path | ✅ extracted runtime seam (`newCloudRuntime`) to keep CLI parse logic clean |
| 6.2 | `cmd/aria-core/main_extra_test.go` | Unit | ✅ same cmd baseline | ✅ runtime constructor/start call absent before test | ✅ cmd package tests green with `cmdCloudServe` | ✅ validated runtime config propagation and start invocation | ➖ none needed |
...
| 6.5 | Docs + runtime smoke artifacts | Integration/Docs | ✅ package tests green before docs/smoke | ✅ docker/local runtime path missing before this batch | ✅ smoke: Postgres container + `go run ./cmd/aria-core cloud serve` with 200 responses for health/push/pull/chunk | ✅ exercised health + mutation + manifest + chunk retrieval end-to-end | ✅ added Docker assets + docs mapping to real runtime paths |
...
- **Commands executed**:
  - `go test ./cmd/aria-core ./internal/cloud/cloudserver -run "TestCmdCloudServeStartsCloudRuntime|TestPrintUsage|TestHandlerMountsDashboardAndHealth|TestHandlerSyncPushPullRoundTrip|TestHandlerReturnsUnauthorizedWhenAuthFails"` (RED then GREEN)
  - `go test ./internal/cloud/auth ./internal/cloud/cloudstore ./internal/cloud/cloudserver ./cmd/aria-core`
  - `go test ./...`
  - `go test -cover ./...`
  - Docker smoke (Postgres + local cloud runtime via `go run ./cmd/aria-core cloud serve`) with observed `health=200 push=200 manifest=200 chunk=200`.
- **Approval tests**: None.
...
- **Commands executed**:
  - `go test ./internal/cloud ./internal/cloud/cloudserver ./cmd/aria-core -run "TestConfigFromEnvCloudHost|TestStartBindsConfiguredHost|TestCmdCloudServeStartsCloudRuntime"` (RED then GREEN)
  - `go test ./internal/cloud/... ./cmd/aria-core -run "TestConfigFromEnvCloudHost|TestStartBindsConfiguredHost|TestCmdCloudServeStartsCloudRuntime|TestHandlerMountsDashboardAndHealth|TestHandlerSyncPushPullRoundTrip|TestHandlerReturnsUnauthorizedWhenAuthFails"`
  - `go test ./...`
  - `go test -cover ./...`
