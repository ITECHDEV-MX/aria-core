...
|---|---|---|
| `cmd/aria-core/main.go` | Modify | Add `cloud` subcommands, config/env parsing, daemon startup wiring, and cloud-aware sync/status entrypoints. |
...
|---|---|---|
| Unit | Cloud CLI parsing/help/errors | Extend `cmd/aria-core/*_test.go` with explicit `aria-core cloud ...` cases and unconfigured no-op regression cases. |
...
2. Add explicit `aria-core cloud ...` commands and help text.
...
- Release/packaging scripts are deferred until CLI contract stabilizes (`aria-core cloud` surface + config semantics).
