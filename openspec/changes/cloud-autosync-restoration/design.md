...
- Proposal: `openspec/changes/cloud-autosync-restoration/proposal.md` (AriaCore `sdd/cloud-autosync-restoration/proposal`)
- Exploration: `openspec/changes/cloud-autosync-restoration/exploration.md` (AriaCore `sdd/cloud-autosync-restoration/explore`)
- Spec: deferred (will land alongside tasks; not required for design)
- Copy source: `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/autosync/manager.go` (451 LOC) + `internal/cloud/remote/transport.go` mutation section (lines 360-453)
...
Timeouts: 30s per request (matches existing `RemoteTransport`). No refresh-token path — ARIA_CORE_CLOUD_TOKEN is static.
...
**Rationale**: Separate constructor makes "which transport for what" explicit. Retry policy matches legacy (aria-core-cloud line 340 `sleepWithJitter`); 404 → `PhaseBackoff` with `reason_code=server_unsupported` fulfills proposal AD-8 (loud auto-disable).
...
**Rationale**: New file keeps the route registration diff small and matches the legacy layout (aria-core-cloud has a separate `cloudserver/mutations.go`). Enrollment check is single-source-of-truth via the existing `ProjectAuthorizer`.

### AD-4: Status adapter

**Choice**: New file `cmd/aria-core/autosync_status.go` defining:
...
**Choice**: Add `tryStartAutosync(s *store.Store, cfg store.Config) (cloudAutosyncManager, context.CancelFunc)` in `cmd/aria-core/main.go` right above `cmdServe`. Body:

1. If `!envBool("ARIA_CORE_CLOUD_AUTOSYNC")` → return nil, nil.
...
1. `ARIA_CORE_CLOUD_AUTOSYNC` — must be truthy (`1|true|yes|on`), else return nil silently.
2. `ARIA_CORE_CLOUD_SERVER` — from env OR `cloud.json` (via `resolveCloudRuntimeConfig`). Empty → log `autosync: cloud server not configured`, return nil.
3. `ARIA_CORE_CLOUD_TOKEN` — env only (the config ignores persisted tokens per existing `resolveCloudRuntimeConfig` at line 370). Empty → log `autosync: cloud token missing`, return nil.
...
| `cmd/aria-core` | `main_extra_test.go` (invert) | No | `TestCmdServeAutosyncLifecycleGating`: asserts autosync STARTS when env set; add healthy-start + graceful-shutdown subtests |
| `cmd/aria-core` | `autosync_status_test.go` (new) | No | Adapter mapping with fake manager + fake fallback |

Integration round-trip (Batch 5): new `cmd/aria-core/autosync_e2e_test.go` spins up `httptest.Server` with real `cloudserver.Handler()`, writes locally, asserts mutation arrives on server within one cycle.
...
| `README.md` | Cloud section | One-line: "Set `ARIA_CORE_CLOUD_AUTOSYNC=1` to enable background sync" |
...
| `cmd/aria-core/main.go` | MODIFIED | Delete fatal; add `tryStartAutosync`; update `newCloudAutosyncManager` seam signature to accept transport |
| `cmd/aria-core/autosync_status.go` | NEW | `autosyncStatusAdapter` + phase→server.SyncStatus mapping |
| `cmd/aria-core/autosync_status_test.go` | NEW | Adapter unit tests |
| `cmd/aria-core/main_extra_test.go` | MODIFIED | Invert `TestCmdServeAutosyncLifecycleGating`; add healthy-start + shutdown subtests |
| `cmd/aria-core/autosync_e2e_test.go` | NEW | End-to-end round-trip via `httptest.Server` (Batch 5) |
...
// cmd/aria-core/autosync_status.go
type autosyncStatusAdapter struct {
...
{"mutations":[{"entity":"observation","entity_key":"...","op":"upsert","payload":{...},"project":"aria-core"}]}
...
| REQ | Test file | Layer |
|---|---|---|
| REQ autosync runs when env set | `cmd/aria-core/main_extra_test.go` | CLI |
| REQ graceful shutdown | `cmd/aria-core/main_extra_test.go` | CLI |
| REQ push round-trip | `cmd/aria-core/autosync_e2e_test.go` + `cloudserver/mutations_test.go` | Integration |
| REQ pull round-trip | `cmd/aria-core/autosync_e2e_test.go` | Integration |
...
| REQ status adapter mapping | `cmd/aria-core/autosync_status_test.go` | Unit |
| REQ local writes unblocked on cloud-down | `cmd/aria-core/autosync_e2e_test.go` | Integration |
...
Deployment note: server endpoints MUST be deployed to `aria-core.condetuti.com` BEFORE any client sets `ARIA_CORE_CLOUD_AUTOSYNC=1`. If not, transport returns 404 → manager enters `PhaseBackoff` with `reason_code=server_unsupported` per AD-8 of the proposal.
...
- Proposal: AriaCore `sdd/cloud-autosync-restoration/proposal` + `openspec/changes/cloud-autosync-restoration/proposal.md`
- Exploration: AriaCore `sdd/cloud-autosync-restoration/explore` + `openspec/changes/cloud-autosync-restoration/exploration.md`
- Legacy manager: `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/autosync/manager.go`
- Legacy transport mutation section: `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/remote/transport.go:360-453`
- Legacy mutation routes: `/Users/alanbuscaglia/work/aria-core-cloud/internal/cloud/cloudserver/cloudserver.go:119-122`
- Integrated stub manager: `/Users/alanbuscaglia/work/aria-core/internal/cloud/autosync/manager.go`
- Integrated stub transport: `/Users/alanbuscaglia/work/aria-core/internal/cloud/remote/transport.go:252-280`
- Integrated fatal: `/Users/alanbuscaglia/work/aria-core/cmd/aria-core/main.go:553-556`
- Integrated cloudserver routes: `/Users/alanbuscaglia/work/aria-core/internal/cloud/cloudserver/cloudserver.go:197-199`
- Server SyncStatusProvider: `/Users/alanbuscaglia/work/aria-core/internal/server/server.go:26-45`
