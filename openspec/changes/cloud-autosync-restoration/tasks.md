...
- [x] 2.2 Copy `aria-core-cloud` manager (451 LOC) and adapt:
  - Update import paths to `github.com/ITECHDEV-MX/aria-core/...`
...
- [x] 3.1 Create `cmd/aria-core/autosync_status_test.go` with failing tests:
...
### 3.2 GREEN — `cmd/aria-core/autosync_status.go`
- [x] 3.2 Create `cmd/aria-core/autosync_status.go`:
...
## Batch 4 — cmd/aria-core wiring + lifecycle test inversion

**REQs satisfied**: REQ-210, REQ-211, REQ-212

### 4.1 RED — Env gating + lifecycle tests
- [x] 4.1 Update/add tests in `cmd/aria-core/main_extra_test.go`:
  ...
  - Add `TestAutosyncGatingTokenMissing` (REQ-211: AUTOSYNC=1, server set, token empty → log contains ARIA_CORE_CLOUD_TOKEN, serve continues)
  - Add `TestAutosyncGatingServerMissing` (REQ-211: AUTOSYNC=1, token set, server empty → log contains ARIA_CORE_CLOUD_SERVER, serve continues)
...
### 4.2 GREEN — `tryStartAutosync` + wiring in `cmd/aria-core/main.go`
- [x] 4.2 Modify `cmd/aria-core/main.go`:
  ...
  - Add `tryStartAutosync(s *server.Server, cfg cloud.RuntimeConfig) (*autosync.Manager, context.CancelFunc)`:
    - Step 1: check `os.Getenv("ARIA_CORE_CLOUD_AUTOSYNC") == "1"` — return nil,nil if not
...
- [x] 5.1 Create `cmd/aria-core/autosync_e2e_test.go`:
...
- [x] 5.2 Update `DOCS.md`: add `## Cloud Autosync` section with env vars table (ARIA_CORE_CLOUD_AUTOSYNC, ARIA_CORE_CLOUD_SERVER, ARIA_CORE_CLOUD_TOKEN), phase table (idle/running/healthy/degraded/paused), reason_code table (transport_failed, server_unsupported, upgrade_paused, auth_required), and troubleshooting guide for server_unsupported
...
| 3.1 | `cmd/aria-core/autosync_status_test.go` | REQ-209, REQ-cloud-sync-status | [ ] | [ ] | [ ] |
| 4.1 | `cmd/aria-core/main_extra_test.go` | REQ-210, REQ-211 | [ ] | [ ] | [ ] |
| 5.1 | `cmd/aria-core/autosync_e2e_test.go` | REQ-212, all REQs E2E | [ ] | [ ] | [ ] |
...
| `cmd/aria-core/autosync_status.go` | CREATE | 3 |
| `cmd/aria-core/autosync_status_test.go` | CREATE | 3 |
| `cmd/aria-core/main.go` | MODIFY (delete fatal, add tryStartAutosync) | 4 |
| `cmd/aria-core/main_extra_test.go` | MODIFY (invert + add tests) | 4 |
| `cmd/aria-core/autosync_e2e_test.go` | CREATE | 5 |
| `DOCS.md` | MODIFY | 5 |
| `README.md` | MODIFY | 5 |
