...
- [x] 2.2 GREEN — `aria-core cloud upgrade doctor`
...
| 2.1/2.2 | `cmd/aria-core/main_extra_test.go` | Unit | ✅ existing cloud CLI baseline test | ✅ Added failing doctor CLI tests | ✅ `go test ./cmd/aria-core -run TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic` | ✅ missing project + deterministic repeated output | ➖ none needed |
...
| 3.3/3.4 | `cmd/aria-core/main_extra_test.go` | Unit | ✅ prior cloud CLI baseline | ✅ Added failing bootstrap/status/rollback tests | ✅ `go test ./cmd/aria-core -run TestCmdCloudUpgradeBootstrapStatusAndRollbackSemantics` | ✅ status parity + rollback boundary + bootstrap --resume acceptance | ➖ none needed |
| Help scenario | `cmd/aria-core/main_extra_test.go` | Unit | ✅ same CLI baseline | ✅ Added failing help workflow test | ✅ `go test ./cmd/aria-core -run TestCmdCloudUpgradeHelpShowsGuidedWorkflow` | ✅ workflow + local-first wording assertions | ➖ none needed |
...
| 4.1/4.2 | `cmd/aria-core/main_test.go`, `internal/sync/sync_test.go` | Unit | ✅ `go test ./cmd/aria-core ./internal/sync` | ✅ Added regression tests asserting `sync --cloud` unchanged with upgrade metadata present | ✅ `go test ./cmd/aria-core ./internal/sync -run 'TestCmdSyncCloudRegressionPreservesLegacyBehaviorWithUpgradeStatePresent|TestCloudSyncExportBehaviorUnchangedWhenUpgradeStateExists'` | ✅ legacy success path + no upgrade-state mutation during sync/export | ✅ additive wiring only; no default behavior flips |

## Test Summary

- Total tests written: 14 new named tests
- Focused suite: `go test ./cmd/aria-core ./internal/store ./internal/sync ./internal/cloud/... ./internal/server`
- Full suite: `go test ./...`
...
- [x] Wired **production snapshot capture** before `bootstrap` progression (`cmd/aria-core/cloud.go`) so rollback always has a pre-upgrade snapshot source.
...
| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|---|---|
| CRIT-1 snapshot capture | `cmd/aria-core/main_extra_test.go` | Unit | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic|TestCmdCloudUpgradeBootstrapStatusAndRollbackSemantics|TestCmdCloudUpgradeHelpShowsGuidedWorkflow'` | ✅ Added failing `bootstrap captures rollback snapshot before progression` | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeBootstrapStatusAndRollbackSemantics/bootstrap captures rollback snapshot before progression'` | ✅ snapshot existence + cloud config + enrollment pre-state checks | ✅ extracted `captureUpgradeSnapshotBeforeBootstrap` helper |
| CRIT-2 policy-denied doctor | `cmd/aria-core/main_extra_test.go` | Unit | ✅ same CLI baseline above | ✅ Added failing `policy denied is surfaced from runtime sync state` | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic/policy denied is surfaced from runtime sync state'` | ✅ deterministic unchanged + policy-denied runtime state paths both asserted | ✅ extracted `cloudUpgradePolicyDenied` helper |
| CRIT-3 REQ-UPGRADE-06 docs evidence | `cmd/aria-core/main_extra_test.go` | Unit | ✅ same CLI baseline above | ✅ Added failing `TestCloudUpgradeDocsMatchHelpAndLocalFirstSemantics` | ✅ `go test ./cmd/aria-core -run TestCloudUpgradeDocsMatchHelpAndLocalFirstSemantics` | ✅ help output + README/DOCS commands + local-first text + deferred plugin docs | ➖ none needed |
...
- `go test ./cmd/aria-core ./internal/store ./internal/sync ./internal/cloud/... ./internal/server`
- `go test ./...`
- `go test -cover ./...`
...
- [x] Added targeted branch tests for upgrade-critical CLI command paths in `cmd/aria-core/cloud.go` via `cmd/aria-core/main_extra_test.go`:
...
| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|---|---|
| Coverage follow-up: CLI upgrade branches | `cmd/aria-core/main_extra_test.go` | Unit | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic|TestCmdCloudUpgradeBootstrapStatusAndRollbackSemantics|TestCmdCloudUpgradeHelpShowsGuidedWorkflow'` | ✅ Added new branch-focused tests; first pass failed on over-asserted apply expectation | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeRepairStatusAndRollbackBranches'` | ✅ repair/status/rollback matrix of success + failure branches | ✅ tightened expectation to documented runtime behavior (apply flag acceptance without forced mutation) |
...
- `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeRepairStatusAndRollbackBranches'`
...
- `go test ./cmd/aria-core ./internal/sync ./internal/cloud/autosync`
- `go test ./...`
- `go test -cover ./...`
- `go test -coverprofile=coverage.out ./... && go tool cover -func=coverage.out | rg 'cmd/aria-core/cloud.go|internal/sync/sync.go|internal/cloud/autosync/manager.go'`
...
| Doctor/bootstrap alignment for real-world failure mode | `cmd/aria-core/main_extra_test.go` | Unit | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic/policy denied is surfaced from runtime sync state'` | ✅ Added failing `legacy payload gaps are surfaced by doctor and block bootstrap preflight` subtest | ✅ `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic/legacy payload gaps are surfaced by doctor and block bootstrap preflight'` | ✅ covers doctor deterministic classification + bootstrap preflight block before orchestration | ➖ none needed |
...
- `go test ./cmd/aria-core -run 'TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic/legacy payload gaps are surfaced by doctor and block bootstrap preflight'`
- `go test ./cmd/aria-core ./internal/store ./internal/sync`
- `go test ./...`
- `go test -cover ./...`
