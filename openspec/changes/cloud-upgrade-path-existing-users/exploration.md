...
6. **Cross-project normalization drift** — project naming variants (`AriaCore` vs `aria-core`) can fragment enrollment/sync state and produce partial bootstrap.
...
### Affected Areas
- `cmd/aria-core/cloud.go` — add guided upgrade command surface (`doctor/repair/migrate/bootstrap/status/rollback`) instead of ad-hoc manual sequencing.
- `cmd/aria-core/main.go` (`cmdSync`, preflight/state transitions) — wire upgrade-aware bootstrap and deterministic reason propagation.
...
- Tests: `cmd/aria-core/main_extra_test.go`, `cmd/aria-core/*_test.go`, `internal/store/store_test.go`, `internal/sync/sync_test.go`, `internal/cloud/cloudserver/cloudserver_test.go`.
...
- **Backwards-compat regression risk** in existing `aria-core sync --cloud` flows; preserve current command behavior and add upgrade flow as additive.
