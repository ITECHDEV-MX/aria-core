...
- [x] 2.1 **RED** Add CLI tests in `cmd/aria-core/main_extra_test.go` for `aria-core cloud upgrade doctor --project <p>` categorized output and exit behavior.
- [x] 2.2 **GREEN** Implement `cloud upgrade doctor` command wiring in `cmd/aria-core/cloud.go` using store/sync contracts only.
...
- [x] 3.3 **RED** Add CLI tests in `cmd/aria-core/main_extra_test.go` for `bootstrap --resume`, `status`, and `rollback` boundary semantics.
- [x] 3.4 **GREEN** Implement `bootstrap|status|rollback` commands in `cmd/aria-core/cloud.go` and integrate `cmd/aria-core/main.go` without changing steady-state `sync --cloud` defaults.
...
- [x] 4.1 **RED** Add regression tests (`cmd/aria-core/main_test.go`, `internal/sync/sync_test.go`) proving existing `aria-core sync --cloud` behavior is unchanged after upgrade additions.
...
- [x] 5.1 Run focused suites: `go test ./cmd/aria-core ./internal/store ./internal/sync ./internal/cloud/... ./internal/server`.
