...
The CLI MUST provide `doctor`, `repair`, `bootstrap`, `status`, and `rollback` under `aria-core cloud upgrade`. It SHALL require explicit `--project` and SHALL keep local SQLite as source of truth.

#### Scenario: Recommended upgrade path is discoverable
- GIVEN an existing local project with no cloud enrollment
- WHEN the user runs `aria-core cloud upgrade --help`
...
- GIVEN a user runs `aria-core cloud upgrade doctor` without `--project`
...
- WHEN `aria-core cloud upgrade doctor --project X` is executed twice
...
- WHEN `aria-core cloud upgrade repair --project X` runs
...
- WHEN `aria-core cloud upgrade bootstrap --project X` is retried
...
- WHEN `aria-core cloud upgrade rollback --project X` runs
