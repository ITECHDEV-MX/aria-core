...
### Out of Scope
- Auto-running cloud onboarding from `aria-core setup ...` or plugin installers.
...
### New Capabilities
- `cloud-upgrade-path`: guided migration of existing local AriaCore projects into opt-in cloud replication with preflight, bootstrap, and rollback semantics.
...
- `aria-core cloud upgrade --project <name> --server <url>` — preflight, persist config, verify auth/policy, enroll, snapshot rollback metadata, and stage first sync.
- `aria-core cloud upgrade --status --project <name>` — read-only readiness + bootstrap progress.
- `aria-core cloud upgrade --rollback --project <name>` — restore pre-upgrade local config/enrollment when bootstrap is incomplete.
...
|------|--------|-------------|
| `cmd/aria-core/cloud.go` | Modified | Add guided upgrade/rollback/status entrypoints. |
