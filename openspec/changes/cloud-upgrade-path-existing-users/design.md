...
Add an additive `aria-core cloud upgrade` command family on top of current `cloud config`, `cloud enroll`, and `sync --cloud`. Local SQLite stays authoritative: upgrade state, rollback snapshots, repair findings, and bootstrap checkpoints are persisted in `internal/store`; `cmd/aria-core` stays a thin CLI adapter; `internal/sync` owns first-bootstrap orchestration; cloud packages only validate, transport, and surface status.

## Architecture Decisions

| Decision | Choice | Alternatives | Rationale |
|---|---|---|---|
| Command shape | `aria-core cloud upgrade {doctor|repair|bootstrap|status|rollback}` with explicit subcommand invocation (`--project` required per operation). No implicit shorthand chain is implemented. | Overload `cloud enroll` / `sync --cloud` | Keeps rollout additive, avoids hidden side effects, and preserves existing low-level commands. |
...
|---|---|---|
| `cmd/aria-core/cloud.go` | Modify | Add upgrade subcommand parsing/help and status/rollback output. |
| `cmd/aria-core/main.go` | Modify | Reuse cloud preflight/status helpers for upgrade-aware reason parity. |
...
|---|---|---|
| Unit | CLI help/flag validation and rollback boundary | Extend `cmd/aria-core/*_test.go`. |
