...
1. **GitHub Security Advisories** (preferred): [Report a vulnerability](https://github.com/ITECHDEV-MX/aria-core/security/advisories/new)
...
AriaCore is a local-first CLI tool that writes to a local SQLite database. The attack surface is intentionally small:
...
- **Out of scope**: issues requiring physical access to the machine where aria-core is installed, or issues that require the attacker to already have access to the user's home directory
