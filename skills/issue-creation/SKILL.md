...
4. **Questions go to [Discussions](https://github.com/ITECHDEV-MX/aria-core/discussions)**, not issues
...
| **AriaCore Version** | Output of `aria-core version` |
...
### Steps to Reproduce
1. Start two aria-core instances pointing to the same DB
...
### Problem Description
When scripting with aria-core, parsing the human-readable output of mem search is fragile. There's no machine-readable output format.

### Proposed Solution
Add a \`--json\` flag to \`aria-core mem search\` that outputs results as JSON.

Example:
\`\`\`bash
aria-core mem search \"auth middleware\" --json
\`\`\`
