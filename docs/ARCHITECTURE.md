...
AriaCore trusts the **agent** to decide what's worth remembering — not a firehose of raw tool calls.

### The Agent Saves, AriaCore Stores
...
3. AriaCore persists to SQLite with FTS5 indexing
...
```
aria-core/
├── cmd/aria-core/main.go              # CLI entrypoint
├── internal/
...
├── plugin/
│   ├── opencode/aria-core.ts          # OpenCode adapter plugin
...
├── skills/                         # Contributor AI skills (repo-wide standards + AriaCore-specific guardrails)
...
```
aria-core setup [agent]      Install/setup agent integration (opencode, claude-code, gemini-cli, codex)
aria-core serve [port]       Start HTTP API server (default: 7437)
aria-core mcp                Start MCP server (stdio transport)
aria-core tui                Launch interactive terminal UI
aria-core search <query>     Search memories
aria-core save <title> <msg> Save a memory
aria-core timeline <obs_id>  Chronological context around an observation
aria-core context [project]  Recent context from previous sessions
aria-core stats              Memory statistics
aria-core export [file]      Export all memories to JSON
aria-core import <file>      Import memories from JSON
aria-core sync               Export new memories as compressed chunk to .aria-core/
aria-core sync --all         Export ALL projects (ignore directory-based filter)
aria-core sync --cloud --project <name>
                          Sync against configured cloud endpoint (project-scoped)
aria-core cloud status       Show cloud runtime/config status
aria-core cloud config --server <url>
                          Configure cloud server URL
aria-core cloud enroll <project>
                          Enroll a project for cloud sync
aria-core cloud serve        Run cloud backend + dashboard
aria-core cloud upgrade <doctor|repair|bootstrap|status|rollback> --project <name>
                          Guided upgrade workflow for existing projects
aria-core projects list      Show all projects with obs/session/prompt counts
aria-core projects consolidate  Interactive merge of similar project names [--all] [--dry-run]
aria-core projects prune     Remove projects with 0 observations [--dry-run]
aria-core obsidian-export    Export memories to Obsidian vault (beta)
aria-core version            Show version
```

Cloud constraints (current behavior):

- Cloud is opt-in replication/shared access; local SQLite remains source of truth.
- `aria-core cloud serve` requires `ARIA_CORE_CLOUD_ALLOWED_PROJECTS` in both token-auth and insecure no-auth mode.
- Authenticated cloud serve requires `ARIA_CORE_CLOUD_TOKEN` + explicit non-default `ARIA_CORE_JWT_SECRET`.
- Insecure local-dev mode (`ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1`) still requires the project allowlist and must not be used in production.

Cloud route/auth split (current behavior):

- Local runtime (`aria-core serve`) exposes local JSON APIs and `GET /sync/status` only.
- Cloud runtime (`aria-core cloud serve`) exposes `GET /health`, `GET /sync/pull`, `GET /sync/pull/{chunkID}`, `POST /sync/push`, and `/dashboard/*`.
...
- In insecure mode (`ARIA_CORE_CLOUD_INSECURE_NO_AUTH=1` with no bearer token), dashboard auth is bypassed and `/dashboard/login` redirects to `/dashboard/`.
...
`internal/cloud/autosync/Manager` is a lease-guarded background goroutine started by `aria-core serve` and `aria-core mcp` when `ARIA_CORE_CLOUD_AUTOSYNC=1`. It implements the local-first invariant: all network I/O happens in its own goroutine and never holds locks shared with the SQLite write path.
...
- [Agent Setup](AGENT-SETUP.md) — connect your agent to AriaCore
