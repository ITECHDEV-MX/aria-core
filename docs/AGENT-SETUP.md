...
AriaCore works with **any MCP-compatible agent**. Pick your agent below.

> Cloud bootstrap automation in agent scripts/plugins is intentionally deferred in this rollout. Use `aria-core cloud ...` manually for now.
>
...
> - `aria-core setup ...` installs MCP/plugin integrations only; it does **not** auto-run `aria-core cloud config/enroll/upgrade`.
...
|-------|-----------|---------------|
| Claude Code | `claude plugin marketplace add Gentleman-Programming/aria-core && claude plugin install aria-core` | [Details](#claude-code) |
| OpenCode | `aria-core setup opencode` | [Details](#opencode) |
| Gemini CLI | `aria-core setup gemini-cli` | [Details](#gemini-cli) |
| Codex | `aria-core setup codex` | [Details](#codex) |
| VS Code | `code --add-mcp '{"name":"aria-core","command":"aria-core","args":["mcp"]}'` | [Details](#vs-code-copilot--claude-code-extension) |
...
| Any MCP agent | `aria-core mcp` (stdio) | [Details](#any-other-mcp-agent) |

### Project auto-detection (important)

**Do not pass `project` to write tools.** AriaCore auto-detects the project from the server's working directory (cwd) using git remote URL, repo root name, or directory basename. Agents that include `project` in `mem_save` or similar calls will have that argument silently discarded.

**Recommended first call:** `mem_current_project` — confirms which project AriaCore detected before you start writing. Returns `project_source` (how it was detected) and `available_projects` (if cwd is ambiguous).
...
> **Prerequisite**: Install the `aria-core` binary first (via [Homebrew](INSTALLATION.md#homebrew-macos--linux), [Windows binary](INSTALLATION.md#windows), [binary download](INSTALLATION.md#download-binary-all-platforms), or [source](INSTALLATION.md#install-from-source-macos--linux)). The plugin needs it for the MCP server and session tracking.

**Recommended: Full setup with one command** — installs the plugin AND registers the MCP server in `opencode.json` automatically:

```bash
aria-core setup opencode
```

This does three things:
1. Copies the plugin to `~/.config/opencode/plugins/aria-core.ts` (session tracking, Memory Protocol, compaction recovery)
2. Adds the `aria-core` MCP server entry to your `opencode.json` with `--tools=agent` (12 agent-facing tools)
...
> **Windows**: OpenCode uses `~/.config/opencode/` on Windows too (it does not read `%APPDATA%\opencode\`). `aria-core setup opencode` writes to `~/.config/opencode/plugins/` and `~/.config/opencode/opencode.json`. To run the server in the background: `Start-Process aria-core -ArgumentList "serve" -WindowStyle Hidden` (PowerShell) or just run `aria-core serve` in a separate terminal.
...
> **Prerequisite**: Install the `aria-core` binary first (via [Homebrew](INSTALLATION.md#homebrew-macos--linux), [Windows binary](INSTALLATION.md#windows), [binary download](INSTALLATION.md#download-binary-all-platforms), or [source](INSTALLATION.md#install-from-source-macos--linux)). The plugin needs it for the MCP server and session tracking scripts.
...
```bash
claude plugin marketplace add Gentleman-Programming/aria-core
claude plugin install aria-core
```

That's it. The plugin registers the MCP server, hooks, and Memory Protocol skill automatically.

**Option B: Plugin via `aria-core setup`** — same plugin, installed from the embedded binary:

```bash
aria-core setup claude-code
```

During setup, you'll be asked whether to add aria-core tools to `~/.claude/settings.json` permissions allowlist — this prevents Claude Code from prompting for confirmation on every memory operation.
...
With bare MCP, add a [Surviving Compaction](#surviving-compaction-recommended) prompt to your `CLAUDE.md` so the agent remembers to use AriaCore after context resets.
...
```bash
aria-core setup gemini-cli
```

`aria-core setup gemini-cli` now does three things:
- Registers `mcpServers.aria-core` in `~/.gemini/settings.json` (Windows: `%APPDATA%\gemini\settings.json`)
- Writes `~/.gemini/system.md` with the AriaCore Memory Protocol (includes post-compaction recovery)
- Ensures `~/.gemini/.env` contains `GEMINI_SYSTEM_MD=1` so Gemini actually loads that system prompt

> `aria-core setup gemini-cli` automatically writes the full Memory Protocol to `~/.gemini/system.md`, so the agent knows exactly when to save, search, and close sessions. No additional configuration needed.
...
```bash
gemini mcp add aria-core aria-core mcp
```
...
```bash
aria-core setup codex
```

`aria-core setup codex` now does three things:
- Registers `[mcp_servers.aria-core]` in `~/.codex/config.toml` (Windows: `%APPDATA%\codex\config.toml`)
- Writes `~/.codex/aria-core-instructions.md` with the AriaCore Memory Protocol
- Writes `~/.codex/aria-core-compact-prompt.md` and points `experimental_compact_prompt_file` to it, so compaction output includes a required memory-save instruction

> `aria-core setup codex` automatically writes the full Memory Protocol to `~/.codex/aria-core-instructions.md` and a compaction recovery prompt to `~/.codex/aria-core-compact-prompt.md`. No additional configuration needed.

Manual alternative: add to your `~/.codex/config.toml` (Windows: `%APPDATA%\codex\config.toml`):

```toml
model_instructions_file = "~/.codex/aria-core-instructions.md"
experimental_compact_prompt_file = "~/.codex/aria-core-compact-prompt.md"
...
3. Add the same `aria-core` server entry above to VS Code User `mcp.json`:
...
```bash
code --add-mcp "{\"name\":\"aria-core\",\"command\":\"aria-core\",\"args\":[\"mcp\"]}"
```
...
> **Windows**: Make sure `aria-core.exe` is in your `PATH`. VS Code resolves MCP commands from the system PATH.
...
- macOS: `~/Library/Application Support/Code/User/prompts/aria-core-memory.instructions.md`
- Linux: `~/.config/Code/User/prompts/aria-core-memory.instructions.md`
- Windows: `%APPDATA%\Code\User\prompts\aria-core-memory.instructions.md`
...
> **Windows**: Make sure `aria-core.exe` is in your `PATH`. Cursor resolves MCP commands from the system PATH.

> **Memory Protocol:** Cursor uses `.mdc` rule files stored in `.cursor/rules/` (Cursor 0.43+). Create an `aria-core.mdc` file (any name works — the `.mdc` extension is what matters) and place it in one of:
> - **Project-specific:** `.cursor/rules/aria-core.mdc` — commit to git so your whole team gets it
> - **Global (all projects):** `~/.cursor/rules/aria-core.mdc` (Windows: `%USERPROFILE%\.cursor\rules\aria-core.mdc`) — create the directory if it doesn't exist
>
...
The pattern is always the same — point your agent's MCP config to `aria-core mcp` via stdio transport.

---

## Surviving Compaction (Recommended)

When your agent compacts (summarizes long conversations to free context), it starts fresh — and might forget about AriaCore. To make memory truly resilient, add this to your agent's system prompt or config file:

**For Claude Code** (`CLAUDE.md`):
```markdown
## Memory
You have access to AriaCore persistent memory via MCP tools (mem_save, mem_search, mem_session_summary, etc.).
...
**For Cursor** (`.cursor/rules/aria-core.mdc` or `~/.cursor/rules/aria-core.mdc`):
...
You have access to AriaCore persistent memory (mem_save, mem_search, mem_context).
...
`aria-core serve` and `aria-core mcp` support continuous background replication to an AriaCore Cloud server. This is **opt-in** and never fatal on missing config.

### Prerequisites

1. A running AriaCore Cloud server (see `docker-compose.cloud.yml` or `aria-core cloud serve`). The server must be a build that includes the mutation endpoints (`POST /sync/mutations/push`, `GET /sync/mutations/pull`). If the server is older, autosync enters `PhaseBackoff` with `reason_code: transport_failed` and logs `server_unsupported` to stderr.
...
```sh
export ARIA_CORE_CLOUD_AUTOSYNC=1          # exact "1" only
export ARIA_CORE_CLOUD_TOKEN=your-token    # bearer token
export ARIA_CORE_CLOUD_SERVER=https://cloud.aria-core.example.com
