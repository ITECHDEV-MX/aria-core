...
> Deferred scope note: plugin-level automatic cloud enrollment/login/upgrade orchestration is not part of this rollout yet. Current cloud flows are CLI-driven (`aria-core cloud ...`).
>
...
```bash
# Install via aria-core (recommended — works from Homebrew or binary install)
aria-core setup opencode

# Or manually: cp plugin/opencode/aria-core.ts ~/.config/opencode/plugins/
```

The plugin auto-starts the HTTP server if it's not already running — no manual `aria-core serve` needed.
...
The plugin:
- **Auto-starts** the aria-core server if not running
- **Auto-imports** git-synced memories from `.aria-core/manifest.json` if present in the project
...
- **Enables** `opencode-subagent-statusline` in `tui.json` or `tui.jsonc` during `aria-core setup opencode`, adding a live sub-agent monitor to OpenCode's sidebar/home footer. To disable it later, remove `"opencode-subagent-statusline"` from the `"plugin"` array in your TUI config and restart OpenCode.
...
# Or via aria-core binary (works from Homebrew or binary install)
...
|---------|----------|--------|
| MCP tools available | 16 default (`aria-core mcp`) | 12 agent-profile tools (`aria-core mcp --tools=agent`) |
...
├── .mcp.json                      # Registers aria-core MCP server
...
1. Ensures the aria-core HTTP server is running
2. Creates a new session via the API
3. Auto-imports git-synced chunks from `.aria-core/manifest.json` (if present)
