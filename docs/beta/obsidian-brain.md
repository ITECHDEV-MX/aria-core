...
> **Status**: Beta — feedback welcome on the [GitHub issues](https://github.com/ITECHDEV-MX/aria-core/issues) tagged `beta:obsidian`.
> **Available since**: `v1.12.0-beta.1`
> **Stability**: Behavior is locked but flag names may evolve before stable release.
> **Side-by-side**: This beta installs as `aria-core-beta` so it doesn't replace your stable `aria-core`. Both binaries share the same `~/.aria-core/aria-core.db`, so memories captured by your stable agent show up in the beta exports automatically.
...
![Brain Graph Preview](https://raw.githubusercontent.com/Gentleman-Programming/aria-core/main/assets/obsidian-brain-graph.png)

---

## Why This Exists

AriaCore already stores every decision, bugfix, and architectural insight your agent makes. But that data lives in a SQLite file. You can search it via the CLI, query it via MCP, browse it in the TUI — but you can't **see the connections**.
...
## Install Side-by-Side (`aria-core-beta`)

The beta ships as a separate binary named `aria-core-beta` so it never touches your stable `aria-core` install. Both binaries read and write the same `~/.aria-core/aria-core.db`, so anything captured by your stable agent is immediately visible to the beta.

Pick the right archive for your platform from the [release page](https://github.com/ITECHDEV-MX/aria-core/releases/tag/v1.12.0-beta.1), then extract and rename:

### macOS (Apple Silicon)

```bash
curl -L https://github.com/ITECHDEV-MX/aria-core/releases/download/v1.12.0-beta.1/aria-core_1.12.0-beta.1_darwin_arm64.tar.gz -o /tmp/aria-core-beta.tar.gz
tar -xzf /tmp/aria-core-beta.tar.gz -C /tmp
sudo mv /tmp/aria-core /usr/local/bin/aria-core-beta
sudo chmod +x /usr/local/bin/aria-core-beta
aria-core-beta version
```

### macOS (Intel)

```bash
curl -L https://github.com/ITECHDEV-MX/aria-core/releases/download/v1.12.0-beta.1/aria-core_1.12.0-beta.1_darwin_amd64.tar.gz -o /tmp/aria-core-beta.tar.gz
tar -xzf /tmp/aria-core-beta.tar.gz -C /tmp
sudo mv /tmp/aria-core /usr/local/bin/aria-core-beta
sudo chmod +x /usr/local/bin/aria-core-beta
aria-core-beta version
...
```bash
curl -L https://github.com/ITECHDEV-MX/aria-core/releases/download/v1.12.0-beta.1/aria-core_1.12.0-beta.1_linux_amd64.tar.gz -o /tmp/aria-core-beta.tar.gz
tar -xzf /tmp/aria-core-beta.tar.gz -C /tmp
sudo mv /tmp/aria-core /usr/local/bin/aria-core-beta
sudo chmod +x /usr/local/bin/aria-core-beta
aria-core-beta version
...
```bash
go install github.com/ITECHDEV-MX/aria-core/cmd/aria-core@v1.12.0-beta.1
mv ~/go/bin/aria-core ~/go/bin/aria-core-beta
aria-core-beta version
```
...
```bash
which aria-core        # /usr/local/bin/aria-core (stable, untouched)
which aria-core-beta   # /usr/local/bin/aria-core-beta (the beta)
aria-core version      # your stable version
aria-core-beta version # v1.12.0-beta.1
```

### Uninstall

```bash
sudo rm /usr/local/bin/aria-core-beta
```

The stable `aria-core`, your `~/.aria-core/aria-core.db`, and your Obsidian vault are untouched.
...
mkdir -p ~/Obsidian/aria-core

# 2. Export your full memory + bootstrap the aria-core-brain graph layout
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --graph-config force

# 3. Open Obsidian, "Open folder as vault" → ~/Obsidian/aria-core
...
SQLite (~/.aria-core/aria-core.db)
 ...
 Transform → Markdown files + YAML frontmatter + wikilinks
        │
        ▼
 {vault}/aria-core/{project}/{type}/{slug}-{id}.md
        │
        ▼
 Obsidian renders the graph from wikilinks
```

Key principle: **the exporter is one-way**. AriaCore writes to the vault, never reads from it. Your manual edits in Obsidian don't propagate back. If you re-run the export, your edited notes are overwritten with the latest from AriaCore. This is by design — the vault is a **live mirror**, not a fork.

The exporter only writes inside `{vault}/aria-core/`. It will never touch other folders in your vault, so you can safely point it at an existing vault that has your own notes.
...
~/Obsidian/aria-core/
├── .obsidian/
│   └── graph.json                      ← aria-core-brain visual config
├── aria-core/                              ← namespace (everything lives here)
│   ├── .aria-core-sync-state.json          ← incremental sync state
...
tags:
  ...
  - sdd
aliases:
  - "Exploration: Obsidian plugin for AriaCore brain visualization"
---
...
```bash
aria-core-beta obsidian-export --vault <path> [flags]
```

| Flag | Required | Default | Description |
|------|----------|---------|-------------|
| `--vault <path>` | ✅ | — | Path to your Obsidian vault root. The exporter writes inside `{vault}/aria-core/` only. |
...
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core

# Export only the aria-core project
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --project aria-core

# Force a complete re-export (useful after pruning observations)
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --force

# Auto-sync every 10 minutes (default), runs forever
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --watch

# Auto-sync every 5 minutes, only the aria-core project
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --watch --interval 5m --project aria-core

# Reset the graph layout to the aria-core-brain default
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --graph-config force

# Never touch the graph config (you have your own custom layout)
aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --graph-config skip
```
...
- Run on a server alongside `aria-core serve` for team-shared brains
...
```bash
nohup ~/go/bin/aria-core-beta obsidian-export \
  --vault ~/Obsidian/aria-core \
  --watch --interval 10m \
  > ~/.aria-core/obsidian-sync.log 2>&1 &
```

**macOS launchd plist** (`~/Library/LaunchAgents/com.aria-core-beta.obsidian-sync.plist`):
```xml
...
<dict>
    <key>Label</key>
    <string>com.aria-core-beta.obsidian-sync</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/bin/aria-core-beta</string>
        <string>obsidian-export</string>
        <string>--vault</string>
        <string>/Users/YOU/Obsidian/aria-core</string>
        <string>--watch</string>
    ...
    <string>/Users/YOU/.aria-core/obsidian-sync.log</string>
...
Then load it:
```bash
launchctl load ~/Library/LaunchAgents/com.aria-core-beta.obsidian-sync.plist
```
...
Obsidian's graph view config lives in `{vault}/.obsidian/graph.json`. AriaCore ships an opinionated default tuned for the aria-core-brain visual style:
...
| `force` | **Always overwrites** `graph.json` with the aria-core-brain default. Use this to reset after experimenting. |
...
| Query | Color | Hex |
|-------|-------|-----|
| `path:aria-core/_sessions` | Soft pink | `#E0CBD2` |
| `path:aria-core/_topics` | Light cyan | `#D3FFFF` |
...
You can absolutely tweak the graph view in Obsidian's UI — AriaCore's `preserve` default mode protects your changes. If you want to start fresh later, run `--graph-config force` once and the aria-core defaults come back.

---

## TypeScript Plugin (Optional)

For an in-Obsidian experience with a ribbon button, settings tab, and status bar indicator, there's a TypeScript community plugin in [`plugin/obsidian/`](https://github.com/ITECHDEV-MX/aria-core/tree/main/plugin/obsidian).

It uses AriaCore's HTTP API (`aria-core serve`) instead of reading SQLite directly:
...
mkdir -p ~/Obsidian/aria-core/.obsidian/plugins/aria-core-brain
cp manifest.json main.js ~/Obsidian/aria-core/.obsidian/plugins/aria-core-brain/
```

Then in Obsidian: Settings → Community plugins → enable "AriaCore Brain". Configure the URL to your `aria-core serve` endpoint (default: `http://127.0.0.1:7437`).

This is **lower priority than the CLI** — the CLI doesn't require `aria-core serve` to be running, works fully offline, and is the recommended path for daily use.
...
| Query | Shows |
|-------|-------|
| `path:aria-core/aria-core` | Only your aria-core project's brain |
| `tag:#bugfix` | Constellation of every bug you've ever fixed |
| `tag:#architecture path:aria-core/aria-core` | Architectural decisions in this project only |
...
- Check that you opened the **vault root** (`~/Obsidian/aria-core`), not the inner `aria-core/aria-core/` folder
...
- Run `aria-core-beta obsidian-export --vault ~/Obsidian/aria-core --graph-config force` to reset to the aria-core-brain default.
...
- `{vault}/aria-core/.aria-core-sync-state.json` — JSON file tracking last export timestamp and per-observation file paths. Safe to delete to force a full re-export, or use `--force`.
...
File issues at [github.com/ITECHDEV-MX/aria-core/issues](https://github.com/ITECHDEV-MX/aria-core/issues) with the `beta:obsidian` label.
...
- **Bidirectional sync** — let Obsidian edits flow back to AriaCore (with conflict resolution)
- **Custom hub templates** — user-defined templates for session/topic hub notes
- **Graph layout presets** — multiple curated layouts beyond the aria-core-brain default
- **`aria-core serve` integration** — auto-export when the HTTP server is running, no separate daemon needed
