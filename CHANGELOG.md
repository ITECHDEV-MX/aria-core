...
All notable changes to AriaCore are documented here.
...
Full release notes with changelogs per version live on the **[GitHub Releases page](https://github.com/ITECHDEV-MX/aria-core/releases)**.
...
Background mutation-based replication for `aria-core serve` and `aria-core mcp`:

- **feat(autosync):** `internal/cloud/autosync.Manager` — lease-guarded background push/pull goroutine enabled by `ARIA_CORE_CLOUD_AUTOSYNC=1` + `ARIA_CORE_CLOUD_TOKEN` + `ARIA_CORE_CLOUD_SERVER`
...
- Use `mem_current_project` (new tool) to inspect which project AriaCore will use before writing.
- If the cwd is ambiguous (multiple git repos), AriaCore returns a structured error with `available_projects`. Navigate to one of the repos before writing.
...
- **feat(cli):** add `aria-core projects list|consolidate|prune` commands for project hygiene
- **feat(mcp):** add `mem_merge_projects` tool for agent-driven project consolidation
- **feat(mcp):** auto-detect project at MCP startup via `--project` flag, `ARIA_CORE_PROJECT` env, or git remote
