...
func TestHandleCapturePassiveDefaultsSourceAndSession(t *testing.T) {
	...
	obs, err := s.RecentObservations("aria-core", "project", 5)
	if err != nil {
...
func TestHandleSearchAndCRUDHandlers(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-mcp", "aria-core", "/tmp/aria-core"); err != nil {
...
func TestHandlePromptContextStatsTimelineAndSessionHandlers(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-flow", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	...
	recent, err := s.RecentObservations("aria-core", "project", 1)
...
func TestMCPHandlersReturnErrorsWhenStoreClosed(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-closed", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	...
	promptRes, err := handleSavePrompt(s, MCPConfig{})(context.Background(), mcppkg.CallToolRequest{Params: mcppkg.CallToolParams{Arguments: map[string]any{"content": "prompt", "project": "aria-core"}}})
	if err != nil {
	...
	sessionSummaryRes, err := handleSessionSummary(s, MCPConfig{}, NewSessionActivity(10*time.Minute))(context.Background(), mcppkg.CallToolRequest{Params: mcppkg.CallToolParams{Arguments: map[string]any{"project": "aria-core", "content": "summary"}}})
	if err != nil {
	...
	sessionStartRes, err := handleSessionStart(s, MCPConfig{}, NewSessionActivity(10*time.Minute))(context.Background(), mcppkg.CallToolRequest{Params: mcppkg.CallToolParams{Arguments: map[string]any{"id": "s1", "project": "aria-core"}}})
	if err != nil {
...
func TestMCPAdditionalCoverageBranches(t *testing.T) {
	...
	if err := s.CreateSession("s-extra", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	firstID, err := s.AddObservation(store.AddObservationParams{SessionID: "s-extra", Type: "note", Title: "first", Content: "first content", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add first: %v", err)
	}
	_, err = s.AddObservation(store.AddObservationParams{SessionID: "s-extra", Type: "note", Title: "second", Content: "second content", Project: "aria-core"})
	if err != nil {
...
func TestHandleUpdateAcceptsAllOptionalFields(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-all-fields", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestHandleContextWithSessionOnlyUsesNoneProjects(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-context-none", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestHandleTimelineBeforeSectionAndSummaryBranches(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-timeline", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	_, err := s.AddObservation(store.AddObservationParams{SessionID: "s-timeline", Type: "note", Title: "first", Content: "first", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add first observation: %v", err)
	}
	focusID, err := s.AddObservation(store.AddObservationParams{SessionID: "s-timeline", Type: "note", Title: "second", Content: "second", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add second observation: %v", err)
	}
	_, err = s.AddObservation(store.AddObservationParams{SessionID: "s-timeline", Type: "note", Title: "third", Content: "third", Project: "aria-core"})
	if err != nil {
...
func TestHandleGetObservationIncludesTopicAndToolMetadata(t *testing.T) {
	s := newMCPTestStore(t)
	if err := s.CreateSession("s-get-meta", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	id, err := s.AddObservation(store.AddObservationParams{
...
func TestNewServerWithToolsNilRegistersAll(t *testing.T) {
	...
	allTools := []string{
		...
		"aria_skills_by_historia", "aria_artifact_sync_to_pages",  // PR2 (#1, #2)
	}
...
func TestNewServerBackwardsCompatible(t *testing.T) {
	...
	if len(tools) != 25 {
		t.Errorf("NewServer should register all 25 tools, got %d", len(tools))
	}
}
...
func TestDefaultSessionIDScopedByProject(t *testing.T) {
	...
	if got := defaultSessionID("aria-core"); got != "manual-save-aria-core" {
		t.Fatalf("expected manual-save-aria-core, got %q", got)
	}
...
func TestNewServerWithConfig(t *testing.T) {
	...
	tools := srv.ListTools()
	// Should have all 25 tools (... + aria_skills_by_historia + aria_artifact_sync_to_pages)
	if len(tools) != 25 {
		t.Errorf("NewServerWithConfig should register all 25 tools, got %d", len(tools))
	}
}
...
func TestHandleSaveSimilarProjectWarning(t *testing.T) {
	...
	// First save: cwd = aria-core repo.
	if err := os.Chdir(ariaCoreDir); err != nil {
		t.Fatal(err)
	}
	req1 := mcppkg.CallToolRequest{Params: mcppkg.CallToolParams{Arguments: map[string]any{
		"title":   "First memory",
		"content": "Memory for aria-core project",
	}}}
	...
	// Second save: cwd = engam repo — should warn about similar "aria-core".
...
func TestHandleMergeProjects(t *testing.T) {
	...
	if err := s.CreateSession("s-AriaCore", "AriaCore", ""); err != nil {
		t.Fatalf("create session AriaCore: %v", err)
	}
	if _, err := s.AddObservation(store.AddObservationParams{
		SessionID: "s-AriaCore",
		Type:      "decision",
		Title:     "From AriaCore",
		Content:   "Content from AriaCore",
		Project:   "aria-core", // store normalizes to lowercase
	}); err != nil {
		t.Fatalf("add observation AriaCore: %v", err)
	}

	if err := s.CreateSession("s-aria-core-memory", "aria-core-memory", ""); err != nil {
		t.Fatalf("create session aria-core-memory: %v", err)
	}
	if _, err := s.AddObservation(store.AddObservationParams{
		SessionID: "s-aria-core-memory",
		Type:      "decision",
		Title:     "From aria-core-memory",
		Content:   "Content from aria-core-memory",
		Project:   "aria-core-memory",
	}); err != nil {
		t.Fatalf("add observation aria-core-memory: %v", err)
	}

	h := handleMergeProjects(s)

	req := mcppkg.CallToolRequest{Params: mcppkg.CallToolParams{Arguments: map[string]any{
		"from": "aria-core-memory, ENGRAM", // comma-separated, with spaces and uppercase
		"to":   "aria-core",
	}}}
	...
	text := callResultText(t, res)
	if !strings.Contains(text, "aria-core") {
	...
	// Verify that aria-core-memory observations are now under "aria-core"
	obs, err := s.RecentObservations("aria-core", "project", 10)
	if err != nil {
		t.Fatalf("recent observations: %v", err)
	}
	// Should have both: original "aria-core" obs + migrated "aria-core-memory" obs
	if len(obs) < 2 {
