...
func parseMustChunk(t *testing.T, payload []byte) coresync.ChunkData {
	t.Helper()
	...
	chunks := []dashboardChunkRow{
		{
			...
			parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntitySession, EntityKey: "s-a1", Op: store.SyncOpUpsert, Payload: `{"id":"s-a1","project":"proj-a","started_at":"2026-04-21T08:00:00Z"}`}}},
		},
		{
			project:   "proj-a",
			createdBy: "alan@example.com",
			createdAt: time.Date(2026, 4, 21, 9, 0, 0, 0, time.UTC),
			parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntityObservation, EntityKey: "obs-a", Op: store.SyncOpUpsert, Payload: `{"sync_id":"obs-a","session_id":"s-a1","project":"proj-a","type":"decision","title":"A","created_at":"2026-04-21T09:00:00Z"}`}}},
		},
		{
			project:   "proj-b",
			createdBy: "alan@example.com",
			createdAt: time.Date(2026, 4, 22, 8, 0, 0, 0, time.UTC),
			parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntitySession, EntityKey: "s-b1", Op: store.SyncOpUpsert, Payload: `{"id":"s-b1","project":"proj-b","started_at":"2026-04-22T08:00:00Z"}`}}},
		},
	}
	...
	base := []dashboardChunkRow{
		{
			...
			createdBy: "alan@example.com",
			createdAt: timestamp,
			parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntityObservation, EntityKey: "obs-1", Op: store.SyncOpUpsert, Payload: `{"sync_id":"obs-1","session_id":"s-1","project":"proj-stable","type":"decision","title":"newer","created_at":"2026-04-23T10:00:00Z"}`}}},
		},
		{
			chunkID:   "a-chunk",
			project:   "proj-stable",
			createdBy: "alan@example.com",
			createdAt: timestamp,
			parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntityObservation, EntityKey: "obs-1", Op: store.SyncOpUpsert, Payload: `{"sync_id":"obs-1","session_id":"s-1","project":"proj-stable","type":"decision","title":"older","created_at":"2026-04-23T09:59:00Z"}`}}},
		},
	}
...
func TestBuildDashboardReadModelFailsOnMalformedMutationPayload(t *testing.T) {
	_, err := buildDashboardReadModel([]dashboardChunkRow{{
		...
		createdBy: "alan@example.com",
		createdAt: time.Date(2026, 4, 23, 10, 0, 0, 0, time.UTC),
		parsed:    coresync.ChunkData{Mutations: []store.SyncMutation{{Entity: store.SyncEntityObservation, EntityKey: "obs-1", Op: store.SyncOpUpsert, Payload: `{"sync_id":`}}},
	}})
	if err == nil {
