...
func (cs *CloudStore) migrate(ctx context.Context) error {
	queries := []string{
		...
		// Sensitivity tag por observation: public | internal | client | confidential.
		// Default 'internal' aplica a las filas pre-existentes; el módulo redactor
		// auto-infiere durante aria_save y permite override explícito vía API.
		...
		// Audit egress: bitácora inmutable de qué se envió a qué LLM y por qué.
		// Cumple bóveda-de-cliente: cualquier salida a LLM externo deja huella.
		...
		// Aliases determinísticos: hash(value) -> [TOKEN-XXXX] -> displayValue.
		// Permite que Claude correlacione múltiples menciones del mismo RFC sin
		// que ARIA pierda la capacidad de Expand inverso para devolverlo al dev.
