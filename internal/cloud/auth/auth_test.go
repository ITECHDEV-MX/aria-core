...
// is the production type used by `aria-core cloud serve` auth wiring.
func TestServiceEnrolledProjects(t *testing.T) {
	svc, err := NewService(&cloudstore.CloudStore{}, strings.Repeat("x", 32))
	if err != nil {
		t.Fatalf("new service: %v", err)
	}
	svc.SetAllowedProjects([]string{"aria-core", "GENTLE-AI", "aria-core"})

	var ep interface{ EnrolledProjects() []string } = svc
	got := ep.EnrolledProjects()
	want := []string{"aria-core", "gentle-ai"}
	if len(got) != len(want) {
