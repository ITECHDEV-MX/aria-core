...
func (s *fakeStore) ReadManifest(context.Context, string) (*coresync.Manifest, error) {
	m := s.manifest
	return &m, nil
}
...
func (s *fakeStore) WriteChunk(_ context.Context, project string, chunkID, createdBy, clientCreatedAt string, payload []byte) error {
	...
	s.manifest.Chunks = append(s.manifest.Chunks, coresync.ChunkEntry{ID: chunkID, CreatedBy: createdBy, CreatedAt: clientCreatedAt})
...
func TestHandlerSyncPushPullRoundTrip(t *testing.T) {
	...
	var manifest coresync.Manifest
...
func TestValidateChunkSessionReferencesAcceptsDeleteMutationWithoutSession(t *testing.T) {
	err := validateChunkSessionReferences(coresync.ChunkData{Mutations: []store.SyncMutation{{
