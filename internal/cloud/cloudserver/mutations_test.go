...
func TestMutationPullUsesEnrollmentProviderWhenImplemented(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, _ = ms.InsertMutationBatch(context.Background(), []MutationEntry{
			{Project: "aria-core", Entity: "obs", EntityKey: fmt.Sprintf("ka%d", i), Op: "upsert", Payload: json.RawMessage(`{}`)},
			{Project: "other-tenant", Entity: "obs", EntityKey: fmt.Sprintf("kb%d", i), Op: "upsert", Payload: json.RawMessage(`{}`)},
		})
	}

	authz := &projectAuthWithEnrollment{token: "secret", enrolled: []string{"aria-core"}}
	...
	if len(resp.Mutations) != 3 {
		t.Fatalf("expected 3 mutations (aria-core only), got %d", len(resp.Mutations))
	}
	for _, m := range resp.Mutations {
		if m.Project != "aria-core" {
			t.Errorf("cross-tenant leak: got project %q, want only \"aria-core\"", m.Project)
		}
	}
	if resp.LatestSeq == 0 {
