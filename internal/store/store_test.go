...
func TestAddObservationDeduplicatesWithinWindow(t *testing.T) {
	...
	if err := s.CreateSession("s1", "aria-core", "/tmp/aria-core"); err != nil {
...
func TestScopeFiltersSearchAndContext(t *testing.T) {
	...
	projectResults, err := s.Search("regex", SearchOptions{Project: "aria-core", Scope: "project", Limit: 10})
	if err != nil {
	...
	personalResults, err := s.Search("regex", SearchOptions{Project: "aria-core", Scope: "personal", Limit: 10})
	if err != nil {
	...
	ctx, err := s.FormatContext("aria-core", "personal")
	if err != nil {
...
func TestUpdateAndSoftDeleteExcludedFromSearchAndTimeline(t *testing.T) {
	...
	searchResults, err := s.Search("deleted", SearchOptions{Project: "aria-core", Limit: 10})
	if err != nil {
...
func TestDifferentTopicsDoNotReplaceEachOther(t *testing.T) {
	...
	observations, err := s.AllObservations("aria-core", "project", 10)
	if err != nil {
...
func TestNewMigratesLegacyObservationIDSchema(t *testing.T) {
	dataDir := t.TempDir()
	dbPath := filepath.Join(dataDir, "aria-core.db")
	...
	_, err = db.Exec(`
		...
		INSERT INTO sessions (id, project, directory) VALUES ('s1', 'aria-core', '/tmp/aria-core');
		INSERT INTO observations (id, session_id, type, title, content, project, created_at)
		VALUES
			(NULL, 's1', 'bugfix', 'legacy null', 'legacy null content', 'aria-core', datetime('now')),
			(7, 's1', 'bugfix', 'legacy fixed', 'legacy fixed content', 'aria-core', datetime('now')),
			(7, 's1', 'bugfix', 'legacy duplicate', 'legacy duplicate content', 'aria-core', datetime('now'));
	`)
	if err != nil {
		_ = db.Close()
	...
	obs, err := s.AllObservations("aria-core", "", 20)
	if err != nil {
	...
	results, err := s.Search("legacy", SearchOptions{Project: "aria-core", Limit: 10})
	if err != nil {
...
func TestNewMigratesLegacyUserPromptsSyncIDSchema(t *testing.T) {
	...
	_, err = db.Exec(`
		...
		INSERT INTO sessions (id, project, directory) VALUES ('s1', 'aria-core', '/tmp/aria-core');
		INSERT INTO user_prompts (session_id, content, project) VALUES ('s1', 'legacy prompt', 'aria-core');
	`)
	if err != nil {
		_ = db.Close()
		t.Fatalf("seed legacy db: %v", err)
...
`
	...
	obs, err := s.AllObservations("aria-core", "", 10)
	if err != nil {
		t.Fatalf("all observations: %v", err)
	}
	if len(obs) != 2 {
...
func TestStatsProjectsOrderedByMostRecentObservation(t *testing.T) {
	...
	if err := s.CreateSession("s2", "aria-core", "/tmp/aria-core"); err != nil {
...
func TestSessionsOrderedByMostRecentActivity(t *testing.T) {
	...
	_, err := s.db.Exec(
		`INSERT INTO sessions (id, project, directory, started_at) VALUES
		 (?, ?, ?, ?),
		 (?, ?, ?, ?)`,
		"s-older", "aria-core", "/tmp/aria-core", "2026-02-01 09:00:00",
		"s-newer", "aria-core", "/tmp/aria-core", "2026-02-02 09:00:00",
	)
	if err != nil {
	...
	_, err = s.db.Exec(
		`INSERT INTO observations (session_id, type, title, content, project, scope, normalized_hash, revision_count, duplicate_count, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?)`,
		"s-older", "note", "latest", "session old got new activity", "aria-core", "project", hashNormalized("session old got new activity"), "2026-02-03 09:00:00", "2026-02-03 09:00:00",
	)
	if err != nil {
...
func TestSessionObservationsAddPromptImportAndSyncChunks(t *testing.T) {
	...
	promptID, err := s.AddPrompt(AddPromptParams{SessionID: "s1", Content: longPrompt, Project: "aria-core"})
	if err != nil {
...
func TestStoreLocalSyncFoundationEnqueuesCoreMutations(t *testing.T) {
	...
	// Enroll "aria-core" so mutations are visible via ListPendingSyncMutations.
	if err := s.EnrollProject("aria-core"); err != nil {
		t.Fatalf("enroll: %v", err)
	}

	if err := s.CreateSession("sync-session", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestApplyRemoteMutationIdempotent(t *testing.T) {
	...
	create := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"id":"remote-session","project":"aria-core","directory":"/remote"}`,
	}
	...
	obsMutation := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"sync_id":"obs-remote-1","session_id":"remote-session","type":"decision","title":"Remote","content":"Pulled from cloud","project":"aria-core","scope":"project"}`,
	}
...
func TestApplyPulledMutationAcceptsStringifiedSessionPayload(t *testing.T) {
	...
	if session.Project != "aria-core" || session.Directory != "/remote" {
...
func TestSessionSyncPayloadPreservesStartedAtOnApply(t *testing.T) {
	...
	if err := s.CreateSession("local-session", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	...
	mutation := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"id":"remote-started-at","project":"aria-core","directory":"/remote","started_at":"2024-01-02 03:04:05"}`,
	}
	if err := s.ApplyPulledMutation(DefaultSyncTargetKey, mutation); err != nil {
...
func TestApplyPulledObservationPreservesChronologyAndRevisionMetadata(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("remote-obs-session", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	...
	mutation := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"sync_id":"obs-meta-1","session_id":"remote-obs-session","type":"decision","title":"meta","content":"preserve metadata","project":"aria-core","scope":"project","created_at":"2024-01-01 00:00:00","updated_at":"2024-01-05 12:30:00","last_seen_at":"2024-01-06 09:15:00","revision_count":7,"duplicate_count":3}`,
	}
	if err := s.ApplyPulledMutation(DefaultSyncTargetKey, mutation); err != nil {
	...
	badChunk := []SyncMutation{
		{
			...
			Op:        SyncOpUpsert,
			Payload:   `{"id":"chunk-session","project":"aria-core","directory":"/remote"}`,
		},
		{
			Entity:    SyncEntityObservation,
			EntityKey: "chunk-obs-bad",
			Op:        SyncOpUpsert,
			Payload:   `{"sync_id":"chunk-obs-bad","session_id":"missing-session","type":"note","title":"bad","content":"fails fk","project":"aria-core","scope":"project"}`,
		},
	}
...
func TestApplyPulledPromptDeleteCreatesTombstoneAndRemovesPrompt(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("s-prompt", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	promptID, err := s.AddPrompt(AddPromptParams{SessionID: "s-prompt", Content: "to-delete", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add prompt: %v", err)
	}
	var syncID string
	...
	mutation := SyncMutation{
		...
		EntityKey: syncID,
		Op:        SyncOpDelete,
		Payload:   fmt.Sprintf(`{"sync_id":"%s","session_id":"s-prompt","project":"aria-core","deleted":true,"hard_delete":true}`, syncID),
	}
	if err := s.ApplyPulledMutation(DefaultSyncTargetKey, mutation); err != nil {
...
func TestApplyPulledPromptUpsertUpdatesCreatedAtOnExistingPrompt(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("s-prompt-upsert", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	promptID, err := s.AddPrompt(AddPromptParams{SessionID: "s-prompt-upsert", Content: "local", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add prompt: %v", err)
	}
	...
	mutation := SyncMutation{
		...
		EntityKey: syncID,
		Op:        SyncOpUpsert,
		Payload:   fmt.Sprintf(`{"sync_id":"%s","session_id":"s-prompt-upsert","content":"remote overwrite","project":"aria-core","created_at":"2024-01-02 03:04:05"}`, syncID),
	}
	if err := s.ApplyPulledMutation(DefaultSyncTargetKey, mutation); err != nil {
...
func TestDeletePromptEnqueuesDeleteMutationAndTombstone(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("s-del-prompt", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	promptID, err := s.AddPrompt(AddPromptParams{SessionID: "s-del-prompt", Content: "bye", Project: "aria-core"})
	if err != nil {
		t.Fatalf("add prompt: %v", err)
	}
	var syncID string
...
func TestDeleteObservationHardDeleteEnqueuesProjectScopedMutationMetadata(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("s-del-obs", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	obsID, err := s.AddObservation(AddObservationParams{
		SessionID: "s-del-obs",
...
func TestDeleteObservationHardDeleteDerivesProjectFromSessionWhenEntityProjectEmpty(t *testing.T) {
	s := newTestStore(t)
	if err := s.CreateSession("s-del-obs-empty", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	obsID, err := s.AddObservation(AddObservationParams{
...
func TestEndSessionAndTimelineDefaults(t *testing.T) {
	...
	if err := s.CreateSession("s-end", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestStoreAdditionalQueryAndMutationBranches(t *testing.T) {
	...
	if err := s.CreateSession("s-q", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestEndSessionEdgeCases(t *testing.T) {
	...
	if err := s.CreateSession("s-edge", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestTimelineHandlesMissingSessionRecord(t *testing.T) {
	...
	res, err := s.db.Exec(
		...
		"manual-save", "manual", "orphan", "orphan content", "aria-core", "project", hashNormalized("orphan content"),
	)
	if err != nil {
...
func TestNewErrorBranches(t *testing.T) {
	...
	t.Run("fails when db path is a directory", func(t *testing.T) {
		dataDir := t.TempDir()
		dbAsDir := filepath.Join(dataDir, "aria-core.db")
	...
	t.Run("legacy migrate surfaces begin and commit hook failures", func(t *testing.T) {
		prepareLegacyStore := func(t *testing.T) *Store {
			...
			if _, err := s.db.Exec(`
				...
				DROP TABLE IF EXISTS observations_fts;
				DROP TABLE observations;
				INSERT OR IGNORE INTO sessions (id, project, directory) VALUES ('s1', 'aria-core', '/tmp/aria-core');
				CREATE TABLE observations (
					id INT,
					session_id TEXT,
					type TEXT,
				...
				VALUES (1, 's1', 'bugfix', 'legacy', 'legacy row', 'aria-core', datetime('now'), datetime('now'));
			`); err != nil {
...
func TestHookFallbacksAndAdditionalBranches(t *testing.T) {
	...
	t.Run("timeline includes before and after in chronological order", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-tl", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}

		firstID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "1", Content: "one", Project: "aria-core"})
		if err != nil {
			t.Fatalf("add first observation: %v", err)
		}
		middleID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "2", Content: "two", Project: "aria-core"})
		if err != nil {
			t.Fatalf("add middle observation: %v", err)
		}
		lastID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "3", Content: "three", Project: "aria-core"})
		if err != nil {
	...
	t.Run("format context returns specific query stage errors", func(t *testing.T) {
		...
		t.Run("recent observations error", func(t *testing.T) {
			s := newTestStore(t)
			if err := s.CreateSession("s-ctx", "aria-core", "/tmp/aria-core"); err != nil {
				t.Fatalf("create session: %v", err)
			}
		...
		t.Run("recent prompts error", func(t *testing.T) {
			s := newTestStore(t)
			if err := s.CreateSession("s-ctx2", "aria-core", "/tmp/aria-core"); err != nil {
				t.Fatalf("create session: %v", err)
			}
...
func TestStoreUncoveredBranchesPushToHundred(t *testing.T) {
	...
	t.Run("add observation, prompt, update forced errors", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-e", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}

		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "top", Content: "x", Project: "aria-core", TopicKey: "x"}); err != nil {
		...
		s.hooks = defaultStoreHooks()
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "dup", Content: "dup content", Project: "aria-core"}); err != nil {
		...
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "x", Content: "y", Project: "aria-core", TopicKey: "t"}); err == nil {
			t.Fatalf("expected topic query error on closed db")
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "x", Content: "y", Project: "aria-core"}); err == nil {
	...
	t.Run("update observation remaining branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-u", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		id, err := s.AddObservation(AddObservationParams{SessionID: "s-u", Type: "old", Title: "t", Content: "c", Project: "aria-core", TopicKey: "topic/key"})
		if err != nil {
	...
	t.Run("query iterator scan and rows.Err branches", func(t *testing.T) {
		...
		if err := s.CreateSession("s-iter", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-iter", Type: "note", Title: "one", Content: "one", Project: "aria-core"}); err != nil {
			t.Fatalf("add observation: %v", err)
		}
		if _, err := s.AddPrompt(AddPromptParams{SessionID: "s-iter", Content: "prompt", Project: "aria-core"}); err != nil {
			t.Fatalf("add prompt: %v", err)
		}
	...
	t.Run("timeline and search type filter branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-t2", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		first, _ := s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "a", Content: "a", Project: "aria-core"})
		_, _ = s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "aa", Content: "aa", Project: "aria-core"})
		focus, _ := s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "b", Content: "b", Project: "aria-core"})
		_, _ = s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "c", Content: "c", Project: "aria-core"})

		if _, err := s.Search("b", SearchOptions{Type: "decision", Project: "aria-core", Scope: "project", Limit: 5}); err != nil {
	...
	t.Run("format context and stats remaining branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-c", "aria-core", "/tmp/aria-core"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-c", Type: "note", Title: "n", Content: "n", Project: "aria-core"}); err != nil {
		...
		if _, err := s.FormatContext("aria-core", "project"); err == nil {
		...
		s.hooks.queryIt = origQueryIt
		ctx, err := s.FormatContext("aria-core", "project")
		if err != nil {
...
func TestEnrollProjectBasic(t *testing.T) {
	...
	if projects[0].Project != "aria-core" {
		t.Fatalf("expected project 'aria-core', got %q", projects[0].Project)
	}
	...
	enrolled, err := s.IsProjectEnrolled("aria-core")
	if err != nil {
...
func TestEnrollAndLookupProjectNormalization(t *testing.T) {
	...
	if err := s.EnrollProject("  ARIA_CORE__CORE  "); err != nil {
	...
	if len(projects) != 1 || projects[0].Project != "aria-core_core" {
		t.Fatalf("expected canonical enrolled project aria-core_core, got %+v", projects)
	}

	enrolled, err := s.IsProjectEnrolled("aria-core__core")
	if err != nil {
	...
	if err := s.UnenrollProject("ARIA_CORE__CORE"); err != nil {
		t.Fatalf("unenroll normalized project: %v", err)
	}
	enrolled, err = s.IsProjectEnrolled("aria-core_core")
	if err != nil {
...
func TestUnenrollProjectBasic(t *testing.T) {
	...
	// Unenroll.
	if err := s.UnenrollProject("aria-core"); err != nil {
...
func TestNormalizeProjectFunction(t *testing.T) {
	tests := []struct {
		...
		{"aria-core", "aria-core", false},
		{"AriaCore", "aria-core", true},
		{"ENGRAM", "aria-core", true},
		{"  aria-core  ", "aria-core", true},
		{"AriaCore-Memory", "aria-core-memory", true},
		{"aria-core--memory", "aria-core-memory", true},
		{"aria-core__memory", "aria-core_memory", true},
		{"", "", false},
...
func TestAddObservationNormalizesProject(t *testing.T) {
	...
	if err := s.CreateSession("s1", "aria-core", "/tmp"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	...
	if obs.Project == nil || *obs.Project != "aria-core" {
		got := "<nil>"
		if obs.Project != nil {
			got = *obs.Project
		}
		t.Errorf("stored project = %q, want \"aria-core\"", got)
	}
}
...
func TestSearchNormalizesProjectFilter(t *testing.T) {
	...
	results, err := s.Search("normalize test", SearchOptions{
		Project: "AriaCore", // intentionally mixed-case
		Limit:   10,
	})
	if err != nil {
...
func TestMergeProjects(t *testing.T) {
	...
	sources := []string{"aria-core", "AriaCore", "aria-core-memory"}
	canonical := "aria-core"

	if err := s.CreateSession("s1", "aria-core", "/work"); err != nil {
		t.Fatalf("create session: %v", err)
	}

	// Add observations to each source
	for _, src := range []string{"aria-core", "aria-core-memory"} {
		for i := 0; i < 2; i++ {
			_, err := s.AddObservation(AddObservationParams{
				SessionID: "s1",
				Type:      "decision",
	...
	if result.Canonical != "aria-core" {
		t.Errorf("canonical = %q, want \"aria-core\"", result.Canonical)
	}

	// "AriaCore" normalizes to "aria-core" (same as canonical) → skipped
	// "aria-core-memory" is different → merged
	// Only "aria-core-memory" should appear in SourcesMerged (and possibly "aria-core" if it had records,
	// but it equals canonical after normalization → skipped)
	for _, merged := range result.SourcesMerged {
		if merged == "aria-core" {
			t.Error("canonical 'aria-core' should not appear in SourcesMerged")
		}
	}

	// All records from aria-core-memory should now be under "aria-core"
	obs, err := s.RecentObservations("aria-core", "", 20)
	if err != nil {
		t.Fatalf("RecentObservations: %v", err)
	}
	if len(obs) < 4 {
		t.Errorf("expected ≥4 observations under 'aria-core' after merge, got %d", len(obs))
	}

	// aria-core-memory should have 0 observations
	obsMerged, err := s.RecentObservations("aria-core-memory", "", 10)
	if err != nil {
		t.Fatalf("RecentObservations aria-core-memory: %v", err)
	}
	if len(obsMerged) != 0 {
		t.Errorf("expected 0 observations under 'aria-core-memory' after merge, got %d", len(obsMerged))
	}
}
...
func TestMergeProjectsIdempotent(t *testing.T) {
	...
	result, err := s.MergeProjects([]string{"ghost-project"}, "aria-core")
	if err != nil {
...
func TestMergeProjectsCanonicalInSources(t *testing.T) {
	...
	result, err := s.MergeProjects([]string{"aria-core", "AriaCore"}, "aria-core")
	if err != nil {
		t.Fatalf("MergeProjects: %v", err)
	}

	// Nothing should have been changed (aria-core and AriaCore both normalize to "aria-core" = canonical)
	if result.ObservationsUpdated != 0 {
...
func TestRepairIsIdempotentOnAlreadyRepairedStore(t *testing.T) {
	...
	if _, err := s.db.Exec(`INSERT OR IGNORE INTO sync_enrolled_projects (project) VALUES (?)`, "aria-core"); err != nil {
		t.Fatalf("enroll project: %v", err)
	}

	// Create a session and observation via the normal API (which also enqueues mutations).
	if err := s.CreateSession("s-repair-1", "aria-core", "/tmp/aria-core"); err != nil {
		t.Fatalf("create session: %v", err)
	}
	if _, err := s.AddObservation(AddObservationParams{
	...
	before := countSyncMutations(t, s, "aria-core")
	if before == 0 {
	...
	after := countSyncMutations(t, s, "aria-core")
	if after != before {
...
func TestRepairBackfillsMissingMutations(t *testing.T) {
	...
	if _, err := s.db.Exec(
		...
		"s-missing", "aria-core", "/tmp/aria-core",
	); err != nil {
		t.Fatalf("insert session: %v", err)
	}
	...
	if _, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, datetime('now'), datetime('now'), ?)`,
		"s-missing", "decision", "T", "C", "aria-core", "project", hashNormalized("C"), obsSync,
	); err != nil {
		t.Fatalf("insert observation: %v", err)
	}
	...
	if _, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, datetime('now'), ?)`,
		"s-missing", "hello", "aria-core", promptSync,
	); err != nil {
		t.Fatalf("insert prompt: %v", err)
	}
