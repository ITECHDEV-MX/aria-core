// Package store implements the persistent memory engine for AriaCore.
//
// It uses SQLite with FTS5 full-text search to store and retrieve
// observations from AI coding sessions. This is the core of AriaCore —
...
// ExportData is the full serializable dump of the aria-core database.
...
func DefaultConfig() (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("aria-core: determine home directory: %w", err)
	}
	return Config{
		DataDir:              filepath.Join(home, ".aria-core"),
...
func New(cfg Config) (*Store, error) {
	if !filepath.IsAbs(cfg.DataDir) {
		return nil, fmt.Errorf("aria-core: data directory must be an absolute path, got %q — set ARIA_CORE_DATA_DIR or ensure your home directory is resolvable", cfg.DataDir)
	}
	if err := os.MkdirAll(cfg.DataDir, 0755); err != nil {
		return nil, fmt.Errorf("aria-core: create data dir: %w", err)
	}

	dbPath := filepath.Join(cfg.DataDir, "aria-core.db")
	db, err := openDB("sqlite", dbPath)
	if err != nil {
		return nil, fmt.Errorf("aria-core: open database: %w", err)
	}
	...
	for _, p := range pragmas {
		if _, err := db.Exec(p); err != nil {
			return nil, fmt.Errorf("aria-core: pragma %q: %w", p, err)
		}
	}

	s := &Store{db: db, cfg: cfg, hooks: defaultStoreHooks()}
	if err := s.migrate(); err != nil {
		return nil, fmt.Errorf("aria-core: migration: %w", err)
	}
	if err := s.repairEnrolledProjectSyncMutations(); err != nil {
		return nil, fmt.Errorf("aria-core: repair enrolled sync journal: %w", err)
	}
...
func (s *Store) Search(query string, opts SearchOptions) ([]SearchResult, error) {
	// Normalize project filter so "AriaCore" finds records stored as "aria-core"
