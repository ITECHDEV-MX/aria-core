...
func TestExtractRepoName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			url:  "git@github.com:ITECHDEV-MX/aria-core.git",
			want: "aria-core",
		},
		{
			name: "HTTPS org with dots",
			url:  "https://github.com/ITECHDEV-MX/aria-core.git",
			want: "aria-core",
		},
		{
...
func TestDetectProject_GitRemote_HTTPS(t *testing.T) {
	...
	cmd := exec.Command("git", "-C", dir, "remote", "add", "origin",
		"https://github.com/ITECHDEV-MX/aria-core.git")
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("git remote add: %v\n%s", err, out)
	}
	...
	got := DetectProject(dir)
	if got != "aria-core" {
		t.Errorf("DetectProject HTTPS remote = %q; want %q", got, "aria-core")
	}
}
