...
func TestInstallGeminiCLIInjectsMCPConfig(t *testing.T) {
	...
	entry, ok := mcpServers["aria-core"].(map[string]any)
	if !ok {
		t.Fatalf("expected mcpServers.aria-core object")
	}

	// Since resolveAriaCoreCommand() uses os.Executable() on all platforms, the
	// command will be the real test binary path in integration tests (not bare
	// "aria-core"). Verify it is a non-empty absolute path.
	cmd, ok := entry["command"].(string)
	if !ok || cmd == "" {
		t.Fatalf("expected non-empty command string, got %#v", entry["command"])
	}
	if cmd == "aria-core" {
		t.Fatalf("expected absolute path from os.Executable(), got bare 'aria-core'")
	}

	args, ok := entry["args"].([]any)
	if !ok || len(args) != 2 || args[0] != "mcp" || args[1] != "--tools=agent" {
		t.Fatalf("expected args [mcp --tools=agent], got %#v", entry["args"])
	}
...
func TestInstallCodexInjectsTOMLAndIsIdempotent(t *testing.T) {
	...
	readAndAssert := func() string {
		...
		if strings.Count(text, "[mcp_servers.aria-core]") != 1 {
			t.Fatalf("expected exactly one aria-core section, got:\n%s", text)
		}
		// resolveAriaCoreCommand() uses os.Executable() on all platforms — command
		// will be the real absolute path in tests, not bare "aria-core".
		if !strings.Contains(text, "command = ") || !strings.Contains(text, "aria-core") {
			t.Fatalf("expected aria-core command in config, got:\n%s", text)
		}
		if !strings.Contains(text, `args = ["mcp", "--tools=agent"]`) {
			t.Fatalf("expected aria-core args in config, got:\n%s", text)
		}
		instructionsPath := filepath.Join(home, ".codex", "aria-core-instructions.md")
		if !strings.Contains(text, "model_instructions_file = \""+instructionsPath+"\"") {
			t.Fatalf("expected model_instructions_file in config, got:\n%s", text)
		}
		compactPromptPath := filepath.Join(home, ".codex", "aria-core-compact-prompt.md")
	...
	instructionsRaw, err := os.ReadFile(filepath.Join(home, ".codex", "aria-core-instructions.md"))
	if err != nil {
	...
	compactRaw, err := os.ReadFile(filepath.Join(home, ".codex", "aria-core-compact-prompt.md"))
	if err != nil {
...
func TestInstallOpenCodeSuccessAndMCPRegistered(t *testing.T) {
	...
	pluginPath := filepath.Join(xdg, "opencode", "plugins", "aria-core.ts")
	...
	if _, ok := mcp["aria-core"]; !ok {
		t.Fatalf("expected mcp.aria-core registration")
	}
...
func TestInstallOpenCodeReadEmbeddedError(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "read embedded aria-core.ts") {
...
func TestInjectOpenCodeMCPPreservesExistingAndIsIdempotent(t *testing.T) {
	...
	entry, ok := mcp["aria-core"].(map[string]any)
	if !ok {
		t.Fatalf("expected aria-core object")
	}
	if entry["enabled"] != true {
		t.Fatalf("expected aria-core.enabled=true")
	}
}
...
func TestInjectOpenCodeMCPConfigErrors(t *testing.T) {
	...
	t.Run("marshal aria-core entry error", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		runtimeGOOS = "linux"
		xdg := filepath.Join(home, "xdg")
		...
		err := injectOpenCodeMCP()
		if err == nil || !strings.Contains(err.Error(), "marshal aria-core entry") {
			t.Fatalf("expected marshal aria-core entry error, got %v", err)
		}
	})
...
func TestInstallClaudeCodeBranches(t *testing.T) {
	...
	t.Run("marketplace already then install success", func(t *testing.T) {
		...
		runCommand = func(_ string, args ...string) ([]byte, error) {
			...
			if strings.Join(args, " ") != "plugin install aria-core" {
...
func TestWriteClaudeCodeUserMCP(t *testing.T) {
	t.Run("writes json with absolute binary path", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/usr/local/bin/aria-core", nil }

		if err := writeClaudeCodeUserMCP(); err != nil {
			t.Fatalf("writeClaudeCodeUserMCP failed: %v", err)
		}

		mcpPath := filepath.Join(home, ".claude", "mcp", "aria-core.json")
		...
		if cfg["command"] != "/usr/local/bin/aria-core" {
	...
	t.Run("overwrites existing (idempotent — always refreshes path)", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/new/path/aria-core", nil }

		mcpDir := filepath.Join(home, ".claude", "mcp")
		if err := os.MkdirAll(mcpDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		if err := os.WriteFile(filepath.Join(mcpDir, "aria-core.json"), []byte(`{"command":"old"}`), 0644); err != nil {
		...
		raw, err := os.ReadFile(filepath.Join(mcpDir, "aria-core.json"))
		if err != nil {
			t.Fatalf("read updated config: %v", err)
		}
		var cfg map[string]any
		...
		if cfg["command"] != "/new/path/aria-core" {
	...
	t.Run("marshal error returns error", func(t *testing.T) {
		resetSetupSeams(t)
		useTestHome(t)
		osExecutable = func() (string, error) { return "/bin/aria-core", nil }
		jsonMarshalIndentFn = func(any, string, string) ([]byte, error) {
	...
	t.Run("write error returns error", func(t *testing.T) {
		...
		home := useTestHome(t)
		osExecutable = func() (string, error) { return "/bin/aria-core", nil }
		// Make ~/.claude/mcp/aria-core.json a directory so write fails
		mcpDir := filepath.Join(home, ".claude", "mcp")
		if err := os.MkdirAll(mcpDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		if err := os.MkdirAll(filepath.Join(mcpDir, "aria-core.json"), 0755); err != nil {
...
func TestResolveAriaCoreCommand(t *testing.T) {
	t.Run("unix returns absolute path from os.Executable", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "/usr/local/bin/aria-core", nil }

		got := resolveAriaCoreCommand()
		// EvalSymlinks on a non-existent path returns an error, so the result
		// is the raw os.Executable() value.
		if got == "aria-core" {
			t.Fatalf("expected absolute path on unix, got bare 'aria-core'")
		}
		if !strings.Contains(got, "aria-core") {
			t.Fatalf("expected aria-core in path, got %q", got)
		}
	})
	...
	t.Run("darwin returns absolute path from os.Executable", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "darwin"
		osExecutable = func() (string, error) { return "/opt/homebrew/bin/aria-core", nil }

		got := resolveAriaCoreCommand()
		if got == "aria-core" {
			t.Fatalf("expected absolute path on darwin, got bare 'aria-core'")
		}
		if !strings.Contains(got, "aria-core") {
			t.Fatalf("expected aria-core in path, got %q", got)
		}
	...
	t.Run("windows returns absolute path", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "windows"
		osExecutable = func() (string, error) { return `C:\Users\user\bin\aria-core.exe`, nil }

		got := resolveAriaCoreCommand()
		// EvalSymlinks may change the path on real OS but in tests it should
		// either equal the input or the resolved form — either way not bare "aria-core"
		if got == "aria-core" {
			t.Fatalf("expected absolute path on windows, got bare 'aria-core'")
		}
		if !strings.Contains(got, "aria-core") {
			t.Fatalf("expected aria-core in path, got %q", got)
		}
	...
	t.Run("executable error falls back to bare name on all platforms", func(t *testing.T) {
		for _, goos := range []string{"linux", "darwin", "windows"} {
			t.Run(goos, func(t *testing.T) {
				...
				if got := resolveAriaCoreCommand(); got != "aria-core" {
					t.Fatalf("expected fallback to bare 'aria-core', got %q", got)
				}
			})
		}
	})
...
func TestClaudeCodeMCPDirPaths(t *testing.T) {
	...
	expectedPath := filepath.Join("/home/tester", ".claude", "mcp", "aria-core.json")
...
func TestGeminiInjectUsesAbsolutePath(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\Users\user\bin\aria-core.exe`},
		{"linux", "/usr/local/bin/aria-core"},
		{"darwin", "/opt/homebrew/bin/aria-core"},
	} {
		t.Run(tc.goos+" uses absolute path", func(t *testing.T) {
			...
			_ = mcpServers; entry := mcpServers["aria-core"].(map[string]any)
			cmd := entry["command"].(string)
			if cmd == "aria-core" {
				t.Fatalf("expected absolute path on %s, got bare 'aria-core'", tc.goos)
			}
			if !strings.Contains(cmd, "aria-core") {
				t.Fatalf("expected aria-core in command path, got %q", cmd)
			}
		})
	}

	t.Run("fallback to bare aria-core when os.Executable fails", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }
		...
		mcpServers := cfg["mcpServers"].(map[string]any)
		_ = mcpServers; entry := mcpServers["aria-core"].(map[string]any)
		if got := entry["command"]; got != "aria-core" {
			t.Fatalf("expected bare 'aria-core' fallback, got %#v", got)
		}
	})
}

// TestCodexBlockUsesAbsolutePath verifies codexAriaCoreBlockStr() always bakes
...
func TestCodexBlockUsesAbsolutePath(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\Users\user\bin\aria-core.exe`, `C:\Users\user\bin\aria-core.exe`},
		{"linux", "/usr/local/bin/aria-core", "/usr/local/bin/aria-core"},
		{"darwin", "/opt/homebrew/bin/aria-core", "/opt/homebrew/bin/aria-core"},
	} {
		t.Run(tc.goos+" uses absolute path in codex block", func(t *testing.T) {
			...
			block := codexAriaCoreBlockStr()
			if !strings.Contains(block, "[mcp_servers.aria-core]") {
				t.Fatalf("expected mcp_servers.aria-core header, got:\n%s", block)
			}
			if !strings.Contains(block, `args = ["mcp", "--tools=agent"]`) {
				t.Fatalf("expected args in codex block, got:\n%s", block)
			}
			if block == codexAriaCoreBlock {
				t.Fatalf("expected absolute path, got bare-aria-core fallback block:\n%s", block)
			}
		})
	}

	t.Run("falls back to bare aria-core when os.Executable fails", func(t *testing.T) {
		resetSetupSeams(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }

		block := codexAriaCoreBlockStr()
		if !strings.Contains(block, `command = "aria-core"`) {
			t.Fatalf("expected bare aria-core fallback in codex block, got:\n%s", block)
		}
	})
}
...
func TestPathHelpersAcrossOSVariants(t *testing.T) {
	...
	if got := codexInstructionsPath(); got != filepath.Join(filepath.Dir(codexConfigPath()), "aria-core-instructions.md") {
		t.Fatalf("unexpected codex instructions path: %s", got)
	}
	if got := codexCompactPromptPath(); got != filepath.Join(filepath.Dir(codexConfigPath()), "aria-core-compact-prompt.md") {
...
func TestGeminiAndCodexHelpersErrorPaths(t *testing.T) {
	t.Run("injectGeminiMCP creates file from missing config", func(t *testing.T) {
		...
		entry, ok := mcpServers["aria-core"].(map[string]any)
		if !ok {
			t.Fatalf("expected aria-core server object")
		}
		// resolveAriaCoreCommand() now returns absolute path on all platforms.
		cmd, ok := entry["command"].(string)
		if !ok || !strings.Contains(cmd, "aria-core") {
			t.Fatalf("expected command containing 'aria-core', got %#v", entry["command"])
		}
	})
	...
	t.Run("upsertCodexAriaCoreBlock replaces section before another section", func(t *testing.T) {
		...
		output := upsertCodexAriaCoreBlock(input)
		if strings.Count(output, "[mcp_servers.aria-core]") != 1 {
			t.Fatalf("expected one aria-core block, got:\n%s", output)
		}
	...
	t.Run("upsertCodexAriaCoreBlock from empty content", func(t *testing.T) {
		resetSetupSeams(t)
		// Force fallback path so output matches the constant.
		osExecutable = func() (string, error) { return "", errors.New("no executable") }

		output := upsertCodexAriaCoreBlock("\n\n")
		if output != codexAriaCoreBlock+"\n" {
...
func TestAdditionalHelperBranches(t *testing.T) {
	...
	t.Run("writeCodexMemoryInstructionFiles compact write error", func(t *testing.T) {
		...
		compactPath := filepath.Join(home, ".codex", "aria-core-compact-prompt.md")
...
func TestInjectOpenCodeMCPHandlesJSONC(t *testing.T) {
  ...
  "mcp": {
	...
	// Verify aria-core was added to the .jsonc file
	...
	if _, ok := mcp["aria-core"]; !ok {
		t.Fatalf("expected aria-core to be registered")
	}
	if _, ok := mcp["other"]; !ok {
...
func TestInjectOpenCodeMCPUsesResolvedCommand(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.goos+" writes absolute path in command array", func(t *testing.T) {
			...
			entry := mcp["aria-core"].(map[string]any)
			...
			if !strings.Contains(first, "aria-core") {
				t.Fatalf("expected aria-core in command path, got %q", first)
			}
	...
	t.Run("executable error falls back to bare aria-core on all platforms", func(t *testing.T) {
		for _, goos := range []string{"linux", "darwin", "windows"} {
			t.Run(goos, func(t *testing.T) {
				resetSetupSeams(t)
				home := useTestHome(t)
				...
				if got := cmd[0].(string); got != "aria-core" {
					t.Fatalf("expected fallback to bare 'aria-core' when os.Executable fails, got %q", got)
				}
			})
		}
	})
...
func TestInstallOpenCodeWarningUsesResolvedCommand(t *testing.T) {
	for _, tc := range []struct {
		...
		{"windows", `C:\bin\aria-core.exe`},
		{"linux", "/nonexistent/bin/aria-core"},  // non-existent so EvalSymlinks is a no-op
		{"darwin", "/nonexistent/bin/aria-core"}, // non-existent so EvalSymlinks is a no-op
	} {
		t.Run(tc.goos+" warning contains absolute path", func(t *testing.T) {
			...
			// Warning must reference the binary path — not just bare "aria-core"
			if !strings.Contains(stderr, "aria-core") {
				t.Fatalf("expected aria-core path in warning on %s, got:\n%s", tc.goos, stderr)
			}
			// Must NOT be the bare "aria-core" unquoted form (since we have an absolute path)
			if strings.Contains(stderr, `["aria-core",`) {
				t.Fatalf("expected absolute path (not bare aria-core) in warning message, got:\n%s", stderr)
			}
		})
	}
}

// ─── Issue #113: OpenCode plugin ARIA_CORE_BIN bake-in ─────────────────────────

// TestPatchAriaCoreBINLine verifies that patchAriaCoreBINLine() correctly rewrites
// the ARIA_CORE_BIN constant in the plugin source to include a Bun.which() runtime
// fallback and a baked-in absolute path as the final headless fallback.
func TestPatchAriaCoreBINLine(t *testing.T) {
	const original = `const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? "aria-core"`

	t.Run("bakes in absolute path with Bun.which intermediate fallback", func(t *testing.T) {
		result := string(patchAriaCoreBINLine([]byte(original), "/usr/local/bin/aria-core"))

		if strings.Contains(result, `?? "aria-core"`) {
			t.Fatalf("original bare-aria-core fallback should be replaced, got:\n%s", result)
		}
		if !strings.Contains(result, `process.env.ARIA_CORE_BIN`) {
			t.Fatalf("must keep process.env.ARIA_CORE_BIN as first option, got:\n%s", result)
		}
		if !strings.Contains(result, `Bun.which("aria-core")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
		if !strings.Contains(result, `"/usr/local/bin/aria-core"`) {
			t.Fatalf("must include baked-in absolute path, got:\n%s", result)
		}
		// Verify precedence order: env var ?? Bun.which ?? absolute path
		envIdx := strings.Index(result, `process.env.ARIA_CORE_BIN`)
		whichIdx := strings.Index(result, `Bun.which`)
		absIdx := strings.Index(result, `"/usr/local/bin/aria-core"`)
	...
	t.Run("Windows path with backslashes is JSON-quoted correctly", func(t *testing.T) {
		result := string(patchAriaCoreBINLine([]byte(original), `C:\Users\user\bin\aria-core.exe`))

		// The path must appear as a properly JSON-escaped string
		if !strings.Contains(result, `Bun.which("aria-core")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
		if !strings.Contains(result, `aria-core.exe`) {
			t.Fatalf("must include Windows binary name, got:\n%s", result)
		}
	})

	t.Run("bare aria-core fallback when os.Executable failed", func(t *testing.T) {
		result := string(patchAriaCoreBINLine([]byte(original), "aria-core"))

		// When absBin=="aria-core", we still add Bun.which but don't repeat "aria-core" as absolute
		if !strings.Contains(result, `process.env.ARIA_CORE_BIN`) {
			t.Fatalf("must keep process.env.ARIA_CORE_BIN, got:\n%s", result)
		}
		if !strings.Contains(result, `Bun.which("aria-core")`) {
			t.Fatalf("must include Bun.which fallback, got:\n%s", result)
		}
	...
	t.Run("does not modify source if marker is absent", func(t *testing.T) {
		src := []byte(`// already patched\nconst ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? Bun.which("aria-core") ?? "/bin/aria-core"`)
		result := patchAriaCoreBINLine(src, "/new/bin/aria-core")
	...
	t.Run("only replaces first occurrence", func(t *testing.T) {
		doubled := original + "\n" + original
		result := string(patchAriaCoreBINLine([]byte(doubled), "/bin/aria-core"))
		// One line should be replaced, the other should remain as-is
		if strings.Count(result, `?? "aria-core"`) != 1 {
...
// plugin file where ARIA_CORE_BIN includes the absolute binary path as a fallback,
...
func TestInstallOpenCodeBakesENGRAMBIN(t *testing.T) {
	t.Run("installed plugin contains absolute path fallback", func(t *testing.T) {
		...
		pluginPath := filepath.Join(home, "xdg", "opencode", "plugins", "aria-core.ts")
		...
		if !strings.Contains(content, `process.env.ARIA_CORE_BIN`) {
			t.Fatalf("installed plugin must keep process.env.ARIA_CORE_BIN override")
		}
		// Must have Bun.which intermediate fallback
		if !strings.Contains(content, `Bun.which("aria-core")`) {
			t.Fatalf("installed plugin must include Bun.which fallback")
		}
		// Must have the baked-in absolute path
		if !strings.Contains(content, `"/usr/local/bin/aria-core"`) {
		...
		srcRaw, err := openCodeReadFile("plugins/opencode/aria-core.ts")
		if err != nil {
			t.Fatalf("read embedded plugin: %v", err)
		}
		if !strings.Contains(string(srcRaw), `?? "aria-core"`) {
			t.Fatalf("source embedded plugin must remain unpatched")
		}
	})

	t.Run("ARIA_CORE_BIN env var still takes precedence at runtime", func(t *testing.T) {
		// We verify by inspection: the installed plugin must use ?? so that a
		// truthy process.env.ARIA_CORE_BIN short-circuits before Bun.which and the
		...
		// const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? Bun.which("aria-core") ?? "/abs/path"
		// where process.env.ARIA_CORE_BIN is leftmost (wins if set).
		envIdx := strings.Index(content, `process.env.ARIA_CORE_BIN`)
		whichIdx := strings.Index(content, `Bun.which("aria-core")`)
		absIdx := strings.Index(content, `"/usr/local/bin/aria-core"`)
		if envIdx == -1 || whichIdx == -1 || absIdx == -1 {
			t.Fatalf("missing expected tokens in installed plugin:\n%s", content)
		}
		if !(envIdx < whichIdx && whichIdx < absIdx) {
			t.Fatalf("wrong operator precedence in ARIA_CORE_BIN line:\n%s", content)
		}
	})

	t.Run("os.Executable fallback: Bun.which added but no double-aria-core", func(t *testing.T) {
		resetSetupSeams(t)
		home := useTestHome(t)
		runtimeGOOS = "linux"
		osExecutable = func() (string, error) { return "", errors.New("no executable") }
