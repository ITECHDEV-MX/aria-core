...
//     (patching ARIA_CORE_BIN to bake in the absolute binary path as a final
...
//     then writes a durable MCP config to ~/.claude/mcp/aria-core.json using the
...
const claudeCodeMarketplace = "ITECHDEV-MX/aria-core"

const openCodeSubagentStatuslinePlugin = "opencode-subagent-statusline"

// claudeCodeMCPTools are the MCP tool names registered by the aria-core plugin
...
var claudeCodeMCPTools = []string{
	"mcp__plugin_aria_core_aria_core__mem_capture_passive",
	"mcp__plugin_aria_core_aria_core__mem_context",
	"mcp__plugin_aria_core_aria_core__mem_get_observation",
	"mcp__plugin_aria_core_aria_core__mem_save",
	"mcp__plugin_aria_core_aria_core__mem_save_prompt",
	"mcp__plugin_aria_core_aria_core__mem_search",
	"mcp__plugin_aria_core_aria_core__mem_session_end",
	"mcp__plugin_aria_core_aria_core__mem_session_start",
	"mcp__plugin_aria_core_aria_core__mem_session_summary",
	"mcp__plugin_aria_core_aria_core__mem_suggest_topic_key",
	"mcp__plugin_aria_core_aria_core__mem_update",
}

// codexAriaCoreBlock is the canonical Codex TOML MCP block.
// Command is always the bare "aria-core" name in this constant because
// upsertCodexAriaCoreBlock generates the actual content via codexAriaCoreBlockStr()
// which uses resolveAriaCoreCommand() at runtime. This constant is kept for tests
// that verify idempotency against the already-written string when os.Executable
// returns "aria-core" (fallback path).
const codexAriaCoreBlock = "[mcp_servers.aria-core]\ncommand = \"aria-core\"\nargs = [\"mcp\", \"--tools=agent\"]"

// codexAriaCoreBlockStr returns the Codex TOML block for the aria-core MCP server,
// using the resolved absolute binary path from os.Executable().
func codexAriaCoreBlockStr() string {
	cmd := resolveAriaCoreCommand()
	return "[mcp_servers.aria-core]\ncommand = " + fmt.Sprintf("%q", cmd) + "\nargs = [\"mcp\", \"--tools=agent\"]"
}

const memoryProtocolMarkdown = `## AriaCore Persistent Memory — Protocol
...
with numbered items. AriaCore will automatically extract and save these as observations.
...
const codexCompactPromptMarkdown = `You are compacting a coding session that uses AriaCore persistent memory.
...
// patchAriaCoreBINLine rewrites the ARIA_CORE_BIN constant declaration in the
...
//
//	const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? Bun.which("aria-core") ?? "/abs/path/aria-core"
//
// Priority (left to right, first truthy wins):
//  1. ARIA_CORE_BIN env var — explicit user override, always respected.
//  2. Bun.which("aria-core") — runtime PATH lookup; works in interactive shells.
//  3. Absolute baked-in path — works in headless/systemd where PATH is stripped.
//
// If absBin is already bare "aria-core" (os.Executable fallback) we don't add it
// as the third fallback because it would be redundant with Bun.which("aria-core").
func patchAriaCoreBINLine(src []byte, absBin string) []byte {
	const marker = `const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? "aria-core"`

	var replacement string
	if absBin == "aria-core" {
		// os.Executable failed — add Bun.which but no baked-in absolute path
		replacement = `const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? Bun.which("aria-core") ?? "aria-core"`
	} else {
		// Normal case: bake in the absolute path as final fallback
		replacement = fmt.Sprintf(
			`const ARIA_CORE_BIN = process.env.ARIA_CORE_BIN ?? Bun.which("aria-core") ?? %q`,
			absBin,
		)
	}
...
func installOpenCode() (*Result, error) {
	...
	data, err := openCodeReadFile("plugins/opencode/aria-core.ts")
	if err != nil {
		return nil, fmt.Errorf("read embedded aria-core.ts: %w", err)
	}

	// Patch ARIA_CORE_BIN in the installed copy so the plugin can find the binary
	...
	// process.env.ARIA_CORE_BIN (explicit user override) and Bun.which("aria-core")
	...
	data = patchAriaCoreBINLine(data, resolveAriaCoreCommand())

	dest := filepath.Join(dir, "aria-core.ts")
	if err := openCodeWriteFileFn(dest, data, 0644); err != nil {
		return nil, fmt.Errorf("write %s: %w", dest, err)
	}

	// Register aria-core MCP server in opencode.json and the subagent monitor in tui.json.
	files := 1
	if err := injectOpenCodeMCPFn(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "  \"aria-core\": { \"type\": \"local\", \"command\": [%q, \"mcp\", \"--tools=agent\"], \"enabled\": true }\n", cmd)
	} else {
		files++
	}
...
// injectOpenCodeMCP adds the aria-core MCP server entry to opencode.json.
// It reads the existing config, adds/updates the aria-core entry under "mcp",
...
func injectOpenCodeMCP() error {
	...
	// Check if aria-core is already registered
	if _, exists := mcpBlock["aria-core"]; exists {
		return nil // already registered, nothing to do
	}

	// Add aria-core MCP entry (agent profile — only tools agents need).
	// Use resolveAriaCoreCommand() so Windows users (and headless Linux setups
	// where PATH is not inherited) get the absolute binary path.
	ariaCoreEntry := map[string]interface{}{
		"type":    "local",
		"command": []string{resolveAriaCoreCommand(), "mcp", "--tools=agent"},
		"enabled": true,
	}
	entryJSON, err := jsonMarshalFn(ariaCoreEntry)
	if err != nil {
		return fmt.Errorf("marshal aria-core entry: %w", err)
	}
	mcpBlock["aria-core"] = json.RawMessage(entryJSON)
...
func installClaudeCode() (*Result, error) {
	...
	installOut, err := runCommand(claudeBin, "plugin", "install", "aria-core")
	...
	// Step 3: Write a durable user-level MCP config at ~/.claude/mcp/aria-core.json
	...
	if err := writeClaudeCodeUserMCPFn(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "warning: could not write user MCP config (~/.claude/mcp/aria-core.json): %v\n", err)
		fmt.Fprintf(os.Stderr, "  The plugin is installed but MCP may not start on Windows if aria-core is not in PATH.\n")
	} else {
		files = 1
	}
...
// claudeCodeUserMCPPath returns the path for the aria-core MCP config in the
// user-level MCP directory.
func claudeCodeUserMCPPath() string {
	return filepath.Join(claudeCodeMCPDir(), "aria-core.json")
}

// writeClaudeCodeUserMCP writes ~/.claude/mcp/aria-core.json with the absolute
// path to the aria-core binary. This is idempotent — it always writes (overwrites)
...
// AddClaudeCodeAllowlist adds aria-core MCP tool names to ~/.claude/settings.json
...
func injectGeminiMCP(configPath string) error {
	...
	ariaCoreEntry := map[string]any{
		"command": resolveAriaCoreCommand(),
		"args":    []string{"mcp", "--tools=agent"},
	}
	entryJSON, err := jsonMarshalFn(ariaCoreEntry)
	if err != nil {
	...
	mcpServers["aria-core"] = json.RawMessage(entryJSON)
...
// resolveAriaCoreCommand returns the absolute path to the aria-core binary.
...
// Falls back to bare "aria-core" only if os.Executable() itself fails.
func resolveAriaCoreCommand() string {
	exe, err := osExecutable()
	if err != nil {
		return "aria-core" // fallback to PATH-based name
	}
	if resolved, err := filepath.EvalSymlinks(exe); err == nil {
		exe = resolved
	}
...
// Previous versions of aria-core added this line, but it causes Gemini CLI to look
...
func injectCodexMCP(configPath string) error {
	...
	updated := upsertCodexAriaCoreBlock(string(data))
...
func upsertCodexAriaCoreBlock(content string) string {
	...
	for i := 0; i < len(lines); {
		...
		if trimmed == "[mcp_servers.aria-core]" {
			i++
			for i < len(lines) {
...
func codexInstructionsPath() string {
	return filepath.Join(filepath.Dir(codexConfigPath()), "aria-core-instructions.md")
}

func codexCompactPromptPath() string {
	return filepath.Join(filepath.Dir(codexConfigPath()), "aria-core-compact-prompt.md")
}
